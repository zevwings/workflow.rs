...
#
#   方式二：本地执行
#   bash 1.5.6-to-1.5.7.sh [--dry-run] [--cleanup]
#
# 选项:
...
if ! command -v python3 &> /dev/null; then
    echo "错误: 需要 Python 3.6+ 环境" >&2
    echo "请安装 Python 3: https://www.python.org/downloads/" >&2
    exit 1
fi

# 检查并安装 toml 库
if ! python3 -c "import toml" 2>/dev/null; then
    echo "ℹ 检测到 toml 库未安装，正在自动安装..."
    if python3 -m pip install --quiet --user toml 2>/dev/null; then
        echo "✓ toml 库安装成功"
    else
        echo "错误: 无法自动安装 toml 库" >&2
        echo "请手动安装: pip install toml" >&2
        exit 1
    fi
fi
