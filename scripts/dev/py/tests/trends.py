...
def analyze(args) -> None:
    ...
    if not metrics:
        ...
        # 即使没有指标数据，也创建一个报告文件，说明情况
        report = _generate_empty_report()
        output_path.write_text(report)
        log_info(f"已创建空报告: {args.output}")
        log_break()
        return
...
def _generate_empty_report() -> str:
    ...
    lines = [
        ...
        '',
        'No test metrics were found for trend analysis.',
        'This may happen if:',
        '- This is the first run',
        '- Test metrics were not collected',
        '- Metrics artifacts were not downloaded',
        ''
    ]
    return '\n'.join(lines)
