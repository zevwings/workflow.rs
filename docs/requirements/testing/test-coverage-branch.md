# Branch 模块测试覆盖率改进计划

> Branch 模块测试覆盖率分析与改进方案

**状态**: 📋 待实施
**当前覆盖率**: 27.9% (76/272 行)
**目标覆盖率**: >80%
**需要提升**: +52.1% (+142 行)
**优先级**: ⭐⭐⭐ 高（核心业务逻辑）

---

## 📋 目录

- [执行摘要](#-执行摘要)
- [当前状态分析](#-当前状态分析)
- [测试覆盖缺失分析](#-测试覆盖缺失分析)
- [测试改进计划](#-测试改进计划)
- [实施优先级](#-实施优先级)
- [相关文档](#-相关文档)

---

## 📊 执行摘要

### 模块概述

Branch 模块是 Workflow CLI 的核心业务模块之一，负责：
- **分支命名生成**：从 JIRA ticket、标题、模板生成分支名称
- **分支同步**：Merge/Rebase 策略的分支同步
- **LLM 集成**：使用 LLM 翻译非英文分支名称
- **分支类型管理**：Feature/Bugfix/Hotfix 等分支类型

### 代码规模

| 指标 | 数值 |
|------|------|
| 总代码行数 | 1,128 行 |
| 可测试行数 | 272 行 |
| 已覆盖行数 | 76 行 |
| 未覆盖行数 | 196 行 |
| 测试代码行数 | 634 行 |

### 当前覆盖率

| 文件 | 覆盖率 | 已覆盖/可测试 | 状态 |
|------|--------|---------------|------|
| `types.rs` | 73.1% | 38/52 | 🟡 良好 |
| `naming.rs` | 27.9% | 38/136 | 🔴 低 |
| `sync.rs` | 0% | 0/144 | 🔴 极低 |
| `llm.rs` | 0% | 0/10 | 🔴 极低 |
| **总计** | **27.9%** | **76/272** | 🔴 **需改进** |

### 核心问题

1. **sync.rs 完全未测试**（0%）：分支同步是核心功能，但完全没有自动化测试
2. **llm.rs 完全未测试**（0%）：LLM 翻译功能缺少测试
3. **naming.rs 覆盖不足**（27.9%）：分支命名逻辑复杂，但测试覆盖不足
4. **测试质量问题**：现有测试主要集中在简单的字符串处理，缺少复杂场景测试

---

## 📈 当前状态分析

### ✅ 已有测试

#### tests/branch/ 目录结构

```
tests/branch/
├── mod.rs              # 模块声明
├── types.rs            # 分支类型测试 ✅
├── naming_utils.rs     # 命名工具函数测试 ✅
└── naming_advanced.rs  # 高级命名测试 ✅
```

#### 已覆盖功能

1. **types.rs (73.1% 覆盖)**：
   - ✅ 分支类型枚举（Feature, Bugfix, Hotfix, Refactoring, Chore）
   - ✅ 类型转换（FromStr, Display）
   - ✅ 类型验证
   - ❌ 部分边界情况未覆盖

2. **naming.rs (27.9% 覆盖)**：
   - ✅ `sanitize()` 基本功能
   - ✅ `slugify()` 基本功能
   - ✅ `from_type_and_slug()` 基本场景
   - ❌ 模板系统集成未测试
   - ❌ LLM 生成未测试
   - ❌ 仓库前缀应用未测试
   - ❌ 复杂场景未测试

### 🚧 未覆盖功能

#### 1. sync.rs (0% 覆盖) - **最高优先级**

**核心功能**：
- 分支同步核心逻辑（Merge/Rebase）
- 工作区状态检查
- Stash 管理
- 冲突处理
- 推送策略

**未测试的关键函数**：
```rust
// 核心同步逻辑
pub fn sync(options, callbacks) -> Result<BranchSyncResult>

// 工作区检查
fn check_working_directory() -> Result<bool>

// 源分支准备
fn prepare_source_branch(source_branch: &str) -> Result<SourceBranchInfo>

// 同步策略确定
fn determine_sync_strategy(rebase, ff_only, squash) -> SyncStrategy

// 执行同步
fn execute_sync(...) -> Result<bool>

// 推送逻辑
fn prompt_and_push_after_sync(result: &BranchSyncResult) -> Result<()>
fn push_after_sync(result: &BranchSyncResult) -> Result<()>
```

**测试难点**：
- 需要 Git 仓库环境
- 需要模拟远程分支
- 需要模拟冲突场景
- 需要模拟用户交互（Dialog）

#### 2. llm.rs (0% 覆盖) - **高优先级**

**核心功能**：
- 使用 LLM 翻译非英文文本为英文

**未测试的关键函数**：
```rust
pub fn translate_to_english(text: &str) -> Result<String>
```

**测试难点**：
- 需要 Mock LLM API 调用
- 需要测试不同语言输入
- 需要测试错误处理

#### 3. naming.rs (27.9% 覆盖) - **中优先级**

**未覆盖的关键函数**：
```rust
// JIRA ticket 生成（模板系统）
pub fn from_jira_ticket(ticket_id, summary, jira_type, use_prefix_format) -> Result<String>

// LLM 生成（fallback）
fn try_llm_generation(ticket_id, summary, use_prefix_format) -> Result<String>

// 仓库前缀应用
fn apply_repo_prefix_if_needed(branch_name: String) -> String

// 格式转换
fn convert_to_double_dash_format(branch_name: &str, ticket_id: &str) -> String

// 清理并翻译
pub fn sanitize_and_translate_branch_name(input: &str) -> Result<String>
```

---

## 🔍 测试覆盖缺失分析

### 1. sync.rs - 分支同步逻辑（144 行未覆盖）

#### 缺失的测试场景

**基础同步场景**：
- [ ] Merge 策略同步（Fast-Forward Only）
- [ ] Merge 策略同步（Squash）
- [ ] Merge 策略同步（Normal Merge）
- [ ] Rebase 策略同步
- [ ] 本地分支同步
- [ ] 远程分支同步

**工作区状态处理**：
- [ ] 干净工作区同步
- [ ] 有未提交更改时 Stash
- [ ] Stash 后恢复
- [ ] 用户取消 Stash

**冲突处理**：
- [ ] Merge 冲突检测
- [ ] Rebase 冲突检测
- [ ] 冲突时恢复 Stash

**推送逻辑**：
- [ ] 远程分支存在时推送
- [ ] 远程分支不存在时跳过
- [ ] Force-with-lease 推送（Rebase 后）
- [ ] 用户取消推送

**回调机制**：
- [ ] 同步成功回调
- [ ] 同步后清理回调
- [ ] 跳过 Stash Pop

**错误处理**：
- [ ] 源分支不存在
- [ ] Fetch 失败
- [ ] Merge 失败
- [ ] Rebase 失败
- [ ] Push 失败

#### 建议的测试文件

```
tests/branch/
└── sync.rs  # 新建：分支同步测试
```

#### 测试策略

1. **使用 Git 测试仓库**：
   - 创建临时 Git 仓库
   - 初始化分支结构
   - 模拟远程仓库

2. **Mock 用户交互**：
   - Mock `SelectDialog` 和 `ConfirmDialog`
   - 预设用户选择

3. **验证 Git 状态**：
   - 检查分支状态
   - 检查提交历史
   - 检查工作区状态

### 2. llm.rs - LLM 翻译功能（10 行未覆盖）

#### 缺失的测试场景

**基础翻译**：
- [ ] 中文翻译为英文
- [ ] 其他语言翻译为英文
- [ ] 已经是英文的输入

**错误处理**：
- [ ] LLM API 调用失败
- [ ] LLM 返回空结果
- [ ] LLM 返回无效格式

**结果清理**：
- [ ] 去除引号
- [ ] 去除多余空格
- [ ] 去除特殊字符

#### 建议的测试文件

```
tests/branch/
└── llm.rs  # 新建：LLM 翻译测试
```

#### 测试策略

1. **Mock LLM Client**：
   - Mock `LLMClient::global()`
   - Mock `client.call()` 返回值

2. **测试不同输入**：
   - 中文输入
   - 混合语言输入
   - 英文输入

3. **测试错误场景**：
   - API 调用失败
   - 空结果
   - 无效格式

### 3. naming.rs - 分支命名逻辑（98 行未覆盖）

#### 缺失的测试场景

**模板系统集成**：
- [ ] 使用模板生成分支名（Feature）
- [ ] 使用模板生成分支名（Bugfix）
- [ ] 模板不存在时 fallback
- [ ] 模板渲染失败时 fallback

**LLM 生成（fallback）**：
- [ ] LLM 生成成功
- [ ] LLM 生成失败，使用简单方法

**仓库前缀**：
- [ ] 应用仓库前缀
- [ ] 已有前缀时不重复
- [ ] 无配置前缀时不添加

**格式转换**：
- [ ] `prefix/ticket-slug` → `ticket--slug`
- [ ] 保留原格式

**清理并翻译**：
- [ ] 非英文输入翻译
- [ ] 英文输入直接清理
- [ ] 翻译失败 fallback
- [ ] 清理后为空的错误处理

**边界情况**：
- [ ] 空字符串
- [ ] 特殊字符
- [ ] 超长字符串
- [ ] Unicode 字符

#### 建议的测试文件

```
tests/branch/
├── naming_template.rs  # 新建：模板系统测试
├── naming_llm.rs       # 新建：LLM 生成测试
└── naming_edge_cases.rs  # 新建：边界情况测试
```

#### 测试策略

1. **Mock 依赖**：
   - Mock `TemplateConfig::load_branch_template()`
   - Mock `TemplateEngine::render_string()`
   - Mock `RepoConfig::get_branch_prefix()`
   - Mock `CreateGenerator::generate()`

2. **测试模板场景**：
   - 不同分支类型
   - 不同模板变量组合
   - 模板错误处理

3. **测试 LLM 场景**：
   - LLM 成功生成
   - LLM 失败 fallback
   - 不同格式输出

---

## 📝 测试改进计划

### 阶段 1：高优先级测试（目标：50% 覆盖率）

#### 1.1 sync.rs 基础测试（预计 +70 行覆盖）

**文件**：`tests/branch/sync.rs`

**测试用例**：
```rust
// 基础同步测试
#[test]
fn test_sync_merge_fast_forward_only() { }

#[test]
fn test_sync_merge_squash() { }

#[test]
fn test_sync_merge_normal() { }

#[test]
fn test_sync_rebase() { }

// 工作区状态测试
#[test]
fn test_sync_with_clean_working_directory() { }

#[test]
fn test_sync_with_uncommitted_changes_stash() { }

// 冲突处理测试
#[test]
fn test_sync_merge_conflict_detection() { }

#[test]
fn test_sync_rebase_conflict_detection() { }

// 推送逻辑测试
#[test]
fn test_sync_push_to_existing_remote_branch() { }

#[test]
fn test_sync_skip_push_for_non_existing_remote() { }
```

**工作量估计**：3-4 天

#### 1.2 llm.rs 完整测试（预计 +10 行覆盖）

**文件**：`tests/branch/llm.rs`

**测试用例**：
```rust
// 基础翻译测试
#[test]
fn test_translate_chinese_to_english() { }

#[test]
fn test_translate_already_english() { }

// 错误处理测试
#[test]
fn test_translate_api_failure() { }

#[test]
fn test_translate_empty_result() { }

// 结果清理测试
#[test]
fn test_translate_result_cleaning() { }
```

**工作量估计**：1 天

### 阶段 2：中优先级测试（目标：70% 覆盖率）

#### 2.1 naming.rs 模板系统测试（预计 +40 行覆盖）

**文件**：`tests/branch/naming_template.rs`

**测试用例**：
```rust
// 模板系统测试
#[test]
fn test_from_jira_ticket_with_template() { }

#[test]
fn test_from_jira_ticket_template_fallback_to_llm() { }

#[test]
fn test_from_jira_ticket_template_fallback_to_simple() { }

// 仓库前缀测试
#[test]
fn test_apply_repo_prefix_when_configured() { }

#[test]
fn test_skip_repo_prefix_when_already_present() { }

// 格式转换测试
#[test]
fn test_convert_to_double_dash_format() { }
```

**工作量估计**：2 天

#### 2.2 naming.rs LLM 生成测试（预计 +20 行覆盖）

**文件**：`tests/branch/naming_llm.rs`

**测试用例**：
```rust
// LLM 生成测试
#[test]
fn test_try_llm_generation_success() { }

#[test]
fn test_try_llm_generation_failure_fallback() { }

// 清理并翻译测试
#[test]
fn test_sanitize_and_translate_non_english() { }

#[test]
fn test_sanitize_and_translate_english() { }

#[test]
fn test_sanitize_and_translate_empty_result_error() { }
```

**工作量估计**：1-2 天

### 阶段 3：完善测试（目标：>80% 覆盖率）

#### 3.1 naming.rs 边界情况测试（预计 +20 行覆盖）

**文件**：`tests/branch/naming_edge_cases.rs`

**测试用例**：
```rust
// 边界情况测试
#[test]
fn test_sanitize_empty_string() { }

#[test]
fn test_sanitize_special_characters() { }

#[test]
fn test_sanitize_very_long_string() { }

#[test]
fn test_sanitize_unicode_characters() { }

#[test]
fn test_slugify_edge_cases() { }
```

**工作量估计**：1 天

#### 3.2 sync.rs 高级场景测试（预计 +30 行覆盖）

**文件**：`tests/branch/sync_advanced.rs`

**测试用例**：
```rust
// 高级场景测试
#[test]
fn test_sync_with_callbacks() { }

#[test]
fn test_sync_skip_stash_pop_when_callback_handled() { }

#[test]
fn test_sync_remote_branch_fetch() { }

// 错误场景测试
#[test]
fn test_sync_source_branch_not_exists() { }

#[test]
fn test_sync_fetch_failure() { }

#[test]
fn test_sync_push_failure() { }
```

**工作量估计**：2 天

#### 3.3 types.rs 完善测试（预计 +14 行覆盖）

**增强现有测试**：
- 添加更多边界情况
- 添加错误处理测试
- 添加类型转换测试

**工作量估计**：0.5 天

---

## 🎯 实施优先级

### P0 - 立即实施（1 周内）

| 任务 | 预计覆盖提升 | 工作量 | 负责人 |
|------|-------------|--------|--------|
| sync.rs 基础测试 | +25.7% | 3-4 天 | TBD |
| llm.rs 完整测试 | +3.7% | 1 天 | TBD |

**预期结果**：覆盖率从 27.9% 提升到 57.3%

### P1 - 短期实施（2 周内）

| 任务 | 预计覆盖提升 | 工作量 | 负责人 |
|------|-------------|--------|--------|
| naming.rs 模板系统测试 | +14.7% | 2 天 | TBD |
| naming.rs LLM 生成测试 | +7.4% | 1-2 天 | TBD |

**预期结果**：覆盖率从 57.3% 提升到 79.4%

### P2 - 中期实施（1 个月内）

| 任务 | 预计覆盖提升 | 工作量 | 负责人 |
|------|-------------|--------|--------|
| naming.rs 边界情况测试 | +7.4% | 1 天 | TBD |
| sync.rs 高级场景测试 | +11.0% | 2 天 | TBD |
| types.rs 完善测试 | +5.1% | 0.5 天 | TBD |

**预期结果**：覆盖率从 79.4% 提升到 >100%（实际约 85-90%，考虑到部分代码难以测试）

---

## 📚 相关文档

### 项目文档

- [测试覆盖度提升综合方案](./test-coverage-improvement.md) - 整体测试覆盖率改进计划
- [测试标准](../../guidelines/testing/README.md) - 项目测试标准和最佳实践
- [开发指南](../../guidelines/development/README.md) - 开发规范和流程

### 架构文档

- [Branch 模块架构](../../architecture/branch.md) - Branch 模块设计文档
- [Git 模块架构](../../architecture/git.md) - Git 操作封装
- [Template 模块架构](../../architecture/template.md) - 模板系统设计

### 源代码

- `src/lib/branch/` - Branch 模块源代码
- `tests/branch/` - Branch 模块测试代码

---

**最后更新**: 2025-12-24

