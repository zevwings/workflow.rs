...
[dependencies]
# 阶段 1: 高优先级
dirs = "5.0"                    # ✅ 已完成 - 路径管理（~10KB，零依赖）
humansize = "2.1"               # ⏳ 待实施 - 文件大小格式化（~8KB，零依赖）

# 阶段 2: 中优先级
reqwest-middleware = "0.2"      # ⏳ 待实施 - HTTP 中间件
reqwest-retry = "0.3"           # ⏳ 待实施 - HTTP 重试策略
tracing = "0.1"                 # ⏳ 待实施 - 结构化日志
tracing-subscriber = "0.3"      # ⏳ 待实施 - 日志订阅者
```
...
| 优先级 | 库 | 影响文件 | 代码减少 | 工作量 | 风险 | 状态 |
|---|---|---|---|---|---|---|---|
| 🔴 高 | `dirs` | paths.rs, logs/path.rs | -30 行 | 1 小时 | 低 | ⚠️ 部分完成 |
| 🔴 高 | `humansize` | format.rs | -35 行 | 0.5 天 | 极低 | ⏳ 待实施 |
| 🟡 中 | `reqwest-retry` | retry.rs | -200 行 | 2 天 | 中 | ⏳ 待实施 |
| 🟡 中 | `tracing` | logger.rs | +50 行 | 1 天 | 低 | ⏳ 待实施 |

### 关键建议

1. **完成 `dirs` crate 深度优化**（⚠️ 优先）
   - 核心已完成，剩余 9 处手动环境变量读取
   - 预计 1 小时完成全部优化
   - 详见：`docs/requirements/dirs-optimization-analysis.md`

2. **立即实施 `humansize`**（🔴 高优先级）
   - 简单替换，收益明显
   - 风险极低，预计 0.5 天

3. **计划阶段 2**（`reqwest-retry` + `tracing`）
   - 需要更多测试
   - 提供显著的功能提升
...
**文档状态**: ✅ 已更新
**当前进度**:
- ✅ `dirs` crate - 已集成，核心功能完成，剩余优化点见 `dirs-optimization-analysis.md`
- ⏳ `humansize` crate - 待实施
- ⏳ 其他库 - 待实施

**下一步行动**:
1. 完成 `dirs` crate 深度优化（剩余 9 处）
2. 引入 `humansize` crate
