...
**职责**：Pre-commit hooks 支持

**核心功能**：
- 检测 pre-commit hooks 是否存在
- 执行 pre-commit hooks
...
1. 在 `branch.rs` 中添加新的 `MergeStrategy` 变体
2. 在 `merge_branch()` 方法中添加对应的处理逻辑
