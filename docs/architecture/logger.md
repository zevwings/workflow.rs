...
Logger 模块是 Workflow CLI 的基础设施模块，为所有模块提供统一的日志记录功能。该模块通过以下方式与其他模块集成：

1. **命令层集成**：所有命令模块使用 `log_*!` 宏输出用户可见的消息，提供友好的用户体验
2. **Lib 层集成**：所有业务逻辑模块使用 `trace_*!` 宏记录内部调试日志，便于问题排查
3. **配置管理**：使用 Settings 模块读取日志级别配置，使用 Paths 获取日志文件路径
4. **应用初始化**：在应用启动时初始化日志系统，确保所有模块都能正常使用日志功能

### 主要集成场景

- **用户输出**：命令层使用 `log_success!`、`log_error!` 等宏输出用户可见的消息
- **调试日志**：Lib 层使用 `trace_debug!`、`trace_info!` 等宏记录内部调试日志
- **日志级别管理**：通过 `LogLevel` 统一管理日志级别，支持环境变量和配置文件配置
...
- [LOG 命令架构文档](../architecture/log.md) - 日志命令模块架构文档
