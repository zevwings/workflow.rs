...
Concurrent 模块是 Workflow CLI 的工具模块，提供通用的并发任务执行功能。该模块通过以下方式与其他模块集成：

1. **Jira 模块集成**：`JiraAttachmentDownloader` 使用 `ConcurrentExecutor` 实现并发下载多个附件，提高下载效率
2. **通用性设计**：使用泛型设计，可以用于任何需要并发执行的同步任务
3. **性能优化**：单任务时直接执行，避免线程开销；多任务时使用线程池并发执行

### 主要集成场景

- **Jira 附件下载**：使用 `ConcurrentExecutor` 并发下载多个附件，显著提高下载速度
- **批量操作**：可以用于批量 API 请求、文件处理、数据处理等场景
...
- [JIRA 模块架构文档](./jira.md) - 使用并发执行器进行附件下载
