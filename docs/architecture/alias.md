...
Alias 模块是 Workflow CLI 的基础模块，提供完整的命令别名管理功能。该模块采用分层架构设计，包括：

- **Lib 层**（`lib/base/alias/`）：提供别名管理的核心业务逻辑，包括别名配置的加载和保存、别名的展开（支持嵌套别名）、别名管理（添加、删除、列表、存在性检查）、循环别名检测和命令行参数展开
- **Commands 层**（`commands/alias/`）：提供 CLI 命令封装，处理用户交互，包括别名列表、添加和删除功能

Alias 模块允许用户为常用命令创建简短别名。别名会在命令解析前自动展开，支持嵌套别名和循环检测。所有命令都提供交互式界面，支持直接模式和交互式模式。

**模块统计：**
- Lib 层代码行数：约 419 行
- Commands 层代码行数：约 342 行
- 命令数量：3 个（list、add、remove）
- 文件数量：Lib 层 3 个，Commands 层 4 个
- 主要依赖：`lib/base/settings/`、`lib/base/dialog/`、`lib/base/util/`

---

## 📁 Lib 层架构（核心业务逻辑）

Alias 模块（`lib/base/alias/`）是 Workflow CLI 的基础库模块，提供别名管理的核心业务逻辑。该模块专注于别名配置的加载和保存、别名的展开（支持嵌套别名）、别名管理（添加、删除、列表、存在性检查）、循环别名检测和命令行参数展开等功能。
...
- **`commands/alias/`**：命令层使用 Lib 层的 API
...
Alias 命令层是 Workflow CLI 的命令接口，提供完整的命令别名管理功能。该层采用命令模式设计，通过调用 `lib/base/alias/` 模块提供的 API 实现业务功能。
...
Alias 命令现在作为 `workflow` 主命令的子命令，通过 `src/bin/workflow.rs` 中的 `Commands::Alias` 枚举定义。
...
Alias 模块采用清晰的分层架构，Lib 层和 Commands 层通过以下方式协作：

1. **别名展开**：Commands 层在命令解析前调用 `AliasManager::expand-_args()` 展开别名
2. **别名管理**：Commands 层调用 Lib 层的 API（`add()`、`remove()`、`list()`）进行别名管理
3. **配置读写**：Lib 层负责配置文件的读写，Commands 层通过 Lib 层 API 间接操作配置
4. **用户交互**：Commands 层处理用户交互（输入、选择、确认），Lib 层专注于业务逻辑
