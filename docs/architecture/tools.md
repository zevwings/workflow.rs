...
- **系统交互**：平台检测、浏览器、剪贴板操作已移至 `lib/base/system/`，请参考相关模块文档
- **文件系统**：文件、路径、目录操作已移至 `lib/base/fs/`，请参考相关模块文档
- **格式化工具**：格式化功能已移至 `lib/base/format/`，请参考 [Format 模块架构文档](./format.md)
- **日志功能**：日志相关功能已移至 `lib/base/logger/`，请参考相关模块文档
- **交互式对话框**：已移至 `lib/base/dialog/`，请参考 [Dialog 模块架构文档](./dialog.md)
- **进度指示器**：已移至 `lib/base/indicator/`，请参考 [Indicator 模块架构文档](./indicator.md)
- **表格输出工具**：已移至 `lib/base/table/`，请参考 [Table 模块架构文档](./table.md)
- **解压工具**：已移至 `lib/base/zip/`，请参考相关模块文档
- **校验和工具**：已移至 `lib/base/checksum/`，请参考相关模块文档

这些工具函数为整个项目提供通用的基础设施支持，被所有模块广泛使用。模块重组提升了代码组织性，同时通过重新导出保持了向后兼容性。
...
**重要变更**：`lib/base/util/` 模块已重组，原来的子模块已迁移到独立的目录，以提升代码组织性和可维护性。

### 当前模块结构

```
src/lib/base/util/
├── mod.rs         # 模块入口，重新导出所有公共 API（保持向后兼容）
└── data/          # 数据处理子模块
    └── mod.rs     # 数据处理相关功能
```
...
以下模块已从 `lib/base/util/` 迁移到独立的目录：

```
src/lib/base/
├── system/        # 系统交互模块（原 util/browser.rs, util/clipboard.rs, util/platform.rs）
│   ├── mod.rs
│   ├── browser.rs      # 浏览器操作
│   ├── clipboard.rs    # 剪贴板操作
│   └── platform.rs     # 平台检测工具
├── fs/            # 文件系统操作模块（原 util/file.rs, util/path.rs, util/directory.rs）
│   ├── mod.rs
│   ├── file.rs         # 文件读写操作
│   ├── path.rs         # 路径处理工具
│   └── directory.rs    # 目录操作工具
├── zip/           # 解压工具模块（原 util/unzip.rs）
│   ├── mod.rs
│   ├── tar.rs          # tar.gz 解压
│   └── zip_impl.rs     # zip 文件解压
├── checksum/      # 校验和工具模块（原 util/checksum.rs）
│   └── mod.rs          # SHA256 哈希计算和验证
├── format/        # 格式化工具模块（原 util/format.rs, util/string.rs, util/date.rs）
│   ├── mod.rs
│   ├── display.rs      # 显示格式化（DisplayFormatter）
│   ├── message.rs      # 消息格式化（MessageFormatter）
│   ├── sensitive.rs    # 敏感字符串处理
│   └── date.rs         # 日期时间格式化
├── logger/        # 日志模块（原 util/logger.rs）
│   ├── mod.rs
│   ├── log_level.rs    # 日志级别管理
│   ├── console.rs      # 控制台日志输出
│   └── tracing.rs      # Tracing 集成
├── dialog/        # 交互式对话框模块（原 util/confirm.rs）
│   ├── mod.rs
│   ├── confirm.rs      # 确认对话框
│   ├── input.rs        # 输入对话框
│   ├── select.rs       # 选择对话框
│   ├── multi_select.rs # 多选对话框
│   └── form/           # 表单对话框
├── indicator/     # 进度指示器模块
│   ├── mod.rs
│   ├── spinner.rs      # 旋转指示器
│   └── progress.rs     # 进度条
└── table/         # 表格输出工具模块（原 util/table.rs）
    └── mod.rs          # 表格构建器和样式
```

### 向后兼容性

`lib/base/util/mod.rs` 通过重新导出保持向后兼容，原有的导入路径仍然可用：
...
// 推荐使用新的导入路径
use workflow::base::fs::{FileReader, PathAccess};
use workflow::base::system::{Browser, Clipboard};
