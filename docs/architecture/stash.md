# Stash 命令模块架构文档

## 📋 概述

本文档描述 Workflow CLI 的 Stash 命令模块架构，包括：
- Stash list 功能（列出所有 stash 条目）
- Stash apply 功能（应用 stash，保留条目）
- Stash drop 功能（删除 stash 条目）
- Stash pop 功能（应用并删除 stash 条目）
- Stash push 功能（保存当前工作区更改到 stash）

Stash 命令模块提供完整的 Git Stash 管理功能，支持查看、应用、删除、保存 stash 条目。所有命令都提供交互式界面，支持选择特定的 stash 条目进行操作。apply 和 pop 命令会自动检测冲突并提供友好的错误提示。

**定位**：命令层专注于用户交互、参数解析和输出格式化，核心业务逻辑由 `lib/git/stash.rs` 模块提供。

**模块统计：**
- 命令数量：5 个（list、apply、drop、pop、push）
- 总代码行数：约 400 行
- 文件数量：6 个
- 主要依赖：`lib/git/stash.rs`、`lib/base/dialog/`、`lib/base/util/`

---

## 📁 Commands 层架构（命令封装）

> **架构说明**：本模块遵循项目的三层架构设计，详见 [architecture.md](./architecture.md#三层架构设计)

### CLI 入口层

```
src/bin/workflow.rs
```
- **职责**：`workflow` 主命令入口，负责命令行参数解析和命令分发
- **功能**：使用 `clap` 解析命令行参数，将 `workflow stash` 子命令分发到对应的命令处理函数

### 命令封装层

```
src/commands/stash/
├── mod.rs          # Stash 命令模块声明（6 行）
├── list.rs         # Stash list 命令（94 行）
├── apply.rs        # Stash apply 命令（83 行）
├── drop.rs         # Stash drop 命令（约 100 行）
├── pop.rs          # Stash pop 命令（72 行）
├── push.rs         # Stash push 命令（56 行）
└── helpers.rs      # 辅助函数（交互式选择 stash）
```

**职责**：
- 解析命令参数
- 处理用户交互（选择、确认等）
- 格式化输出（表格显示）
- 调用核心业务逻辑层 (`lib/git/stash.rs`) 的功能
- 处理冲突和错误情况

### 依赖模块

命令层通过调用 `lib/` 模块提供的 API 实现功能，具体实现细节请参考相关模块文档：
- **`lib/git/stash.rs`**：Git Stash 操作
  - `GitStash::stash-_list()` - 列出所有 stash 条目
  - `GitStash::stash-_push()` - 保存当前工作区更改到 stash
  - `GitStash::stash-_apply()` - 应用 stash（不删除）
  - `GitStash::stash-_drop()` - 删除 stash
  - `GitStash::stash-_pop()` - 应用并删除 stash
  - `GitStash::stash-_show-_stat()` - 获取 stash 统计信息
  - `GitStash::has-_unmerged()` - 检查是否有冲突
- **`lib/base/dialog/`**：用户交互对话框
  - `ConfirmDialog` - 确认对话框
  - `SelectDialog` - 选择对话框
  - `MultiSelectDialog` - 多选对话框
- **`lib/base/util/`**：工具函数
  - `TableBuilder` - 表格构建器
  - `TableStyle` - 表格样式

详细架构文档：参见 [Git 模块架构文档](../architecture/git.md)

---

## 🔄 集成关系

Stash 命令模块是 Workflow CLI 的 Git 暂存管理命令，提供 Git stash 的完整操作功能。该命令通过以下方式与 Lib 层集成：

1. **Git 模块集成**：使用 `GitStash` 进行所有 stash 操作，包括列表、推送、应用、删除、弹出等
2. **Dialog 模块集成**：使用 `ConfirmDialog` 和 `SelectDialog` 进行用户交互
3. **工具模块集成**：使用 `TableBuilder` 和 `TableStyle` 格式化输出

### 主要集成场景

- **Stash 列表**：使用 `GitStash::stash-_list()` 列出所有 stash 条目
- **Stash 操作**：使用 `GitStash::stash-_push()`、`stash-_apply()`、`stash-_drop()`、`stash-_pop()` 进行 stash 操作
- **冲突处理**：使用 `GitStash::has-_unmerged()` 检查冲突

---

## 🔄 调用流程

### 整体架构流程

```
用户输入
  ↓
src/bin/workflow.rs (workflow 主命令，参数解析)
  ↓
commands/stash/*.rs (命令封装层，处理交互)
  ↓
lib/git/stash.rs (业务逻辑层，Git 操作)
  ↓
Git 命令执行
```

### 各命令详细流程

#### 1. `stash list` 命令流程

```
用户输入: workflow stash list [--stat]
  ↓
StashListCommand::execute(show-_stat)
  ↓
GitStash::stash-_list() → 获取所有 stash 条目
  ↓
格式化表格数据 (StashRow)
  ↓
TableBuilder 构建表格
  ↓
显示表格（如果 --stat，显示统计信息）
```

#### 2. `stash apply` 命令流程

```
用户输入: workflow stash apply
  ↓
StashApplyCommand::execute()
  ↓
GitStash::stash-_list() → 获取所有 stash 条目
  ↓
显示最新 stash 信息，询问是否应用
  ↓
如果用户选择其他，调用 select-_stash-_interactively()
  ↓
GitStash::stash-_apply(stash-_ref) → 应用 stash
  ↓
检查冲突 (GitStash::has-_unmerged())
  ↓
显示结果（成功/冲突/错误）
```

#### 3. `stash drop` 命令流程

```
用户输入: workflow stash drop
  ↓
StashDropCommand::execute()
  ↓
GitStash::stash-_list() → 获取所有 stash 条目
  ↓
MultiSelectDialog 多选要删除的 stash
  ↓
显示将要删除的 stash 列表
  ↓
确认删除
  ↓
GitStash::stash-_drop(stash-_ref) → 删除每个选中的 stash
  ↓
显示删除结果
```

#### 4. `stash pop` 命令流程

```
用户输入: workflow stash pop
  ↓
StashPopCommand::execute()
  ↓
GitStash::stash-_list() → 获取所有 stash 条目
  ↓
显示最新 stash 信息，询问是否应用
  ↓
如果用户选择其他，调用 select-_stash-_interactively()
  ↓
GitStash::stash-_pop(stash-_ref) → 应用并删除 stash
  ↓
检查冲突 (GitStash::has-_unmerged())
  ↓
如果冲突，保留 stash 条目
  ↓
显示结果（成功/冲突/错误）
```

#### 5. `stash push` 命令流程

```
用户输入: workflow stash push
  ↓
StashPushCommand::execute()
  ↓
GitCommit::has-_commit() → 检查是否有未提交的更改
  ↓
如果没有更改，显示警告并返回
  ↓
InputDialog 提示输入 stash 消息（可选）
  ↓
GitStash::stash-_push(message) → 保存更改到 stash
  ↓
显示成功消息
```

---

## 功能说明

### 1. `stash list` - 列出所有 stash

**功能描述**：
列出所有 stash 条目，以表格形式显示索引、消息、分支和创建时间。

**命令格式**：
```bash
workflow stash list              # 列出所有 stash
workflow stash list --stat       # 显示统计信息
```

**实现细节**：
- 使用 `GitStash::stash-_list()` 获取所有 stash 条目
- 使用 `TableBuilder` 构建表格，显示索引、消息、分支、创建时间
- 如果指定 `--stat`，使用 `GitStash::stash-_show-_stat()` 获取每个 stash 的文件变更统计

**输出示例**：
```
Stash Entries
┌─────────────┬──────────────────────┬──────────┬─────────────────────┐
│ #           │ Message              │ Branch   │ Created             │
├─────────────┼──────────────────────┼──────────┼─────────────────────┤
│ stash@{0}   │ WIP: feature branch  │ feature  │ 2025-01-27 10:30:00 │
│ stash@{1}   │ Temporary changes     │ main     │ 2025-01-26 15:20:00 │
└─────────────┴──────────────────────┴──────────┴─────────────────────┘
```

### 2. `stash apply` - 应用 stash

**功能描述**：
应用指定的 stash，保留 stash 条目（不删除）。

**命令格式**：
```bash
workflow stash apply              # 交互式选择要应用的 stash
```

**实现细节**：
- 首先显示最新的 stash 信息，询问用户是否应用
- 如果用户选择其他，使用 `select-_stash-_interactively()` 交互式选择
- 使用 `GitStash::stash-_apply()` 应用 stash
- 自动检测冲突（`GitStash::has-_unmerged()`）
- 如果成功，显示文件变更统计；如果有冲突，显示冲突提示

**冲突处理**：
- 如果应用时发生冲突，stash 条目保留
- 显示冲突提示和解决步骤

### 3. `stash drop` - 删除 stash

**功能描述**：
删除一个或多个 stash 条目。

**命令格式**：
```bash
workflow stash drop               # 交互式选择要删除的 stash
```

**实现细节**：
- 使用 `MultiSelectDialog` 多选要删除的 stash
- 显示将要删除的 stash 列表供用户确认
- 使用 `GitStash::stash-_drop()` 删除每个选中的 stash
- 显示删除结果

**安全机制**：
- 多选确认，避免误删
- 显示将要删除的 stash 列表
- 二次确认机制

### 4. `stash pop` - 应用并删除 stash

**功能描述**：
应用指定的 stash 并删除该 stash 条目。

**命令格式**：
```bash
workflow stash pop                # 交互式选择要应用的 stash
```

**实现细节**：
- 首先显示最新的 stash 信息，询问用户是否应用
- 如果用户选择其他，使用 `select-_stash-_interactively()` 交互式选择
- 使用 `GitStash::stash-_pop()` 应用并删除 stash
- 自动检测冲突（`GitStash::has-_unmerged()`）
- **重要**：如果应用失败（冲突），stash 条目保留，不删除

**冲突处理**：
- 如果应用时发生冲突，stash 条目保留（不删除）
- 显示冲突提示和解决步骤
- 用户解决冲突后可以再次尝试

### 5. `stash push` - 保存当前更改到 stash

**功能描述**：
将当前工作区和暂存区的未提交更改保存到 stash。支持可选的 stash 消息来标识这次保存的内容。

**命令格式**：
```bash
workflow stash push                # 保存当前更改到 stash（提示输入消息）
```

**实现细节**：
- 首先检查工作区是否有未提交的更改（`GitCommit::has-_commit()`）
- 如果没有更改，显示警告并返回
- 使用 `InputDialog` 提示用户输入可选的 stash 消息
- 使用 `GitStash::stash-_push()` 保存更改到 stash
- 显示成功消息（包含消息内容，如果有）

**使用场景**：
- 临时保存当前工作进度，切换到其他分支
- 在切换分支前保存未提交的更改
- 保存实验性更改，稍后恢复

---

## 📊 数据流

### Stash 数据结构

```rust
// lib/git/stash.rs
pub struct StashEntry {
    pub index: usize,              // stash@{n} 中的 n
    pub branch: String,             // 创建时的分支
    pub message: String,             // stash 消息
    pub commit-_hash: String,        // commit hash
    pub timestamp: Option<DateTime<Local>>,  // 创建时间
}

pub struct StashApplyResult {
    pub applied: bool,               // 是否成功应用
    pub has-_conflicts: bool,         // 是否有冲突
    pub message: Option<String>,     // 消息
    pub warnings: Vec<String>,       // 警告列表
    pub stat: Option<StashStat>,     // 统计信息
}

pub struct StashStat {
    pub files-_changed: usize,        // 变更的文件数
    pub insertions: usize,           // 插入的行数
    pub deletions: usize,            // 删除的行数
}
```

### 数据流向

```
Git 命令输出
  ↓
GitStash::stash-_list() → Vec<StashEntry>
  ↓
命令层格式化 → 表格显示
```

---

## 🔗 与其他模块的集成

### 与 Git 模块的集成

- **`lib/git/stash.rs`**：提供所有 stash 操作的底层实现
- 使用 `git stash` 命令执行实际操作
- 提供冲突检测功能

### 与 Dialog 模块的集成

- **`lib/base/dialog/`**：提供交互式对话框
- `ConfirmDialog`：确认操作
- `SelectDialog`：单选 stash
- `MultiSelectDialog`：多选 stash（用于 drop 命令）

### 与 Util 模块的集成

- **`lib/base/util/`**：提供工具函数
- `TableBuilder`：构建表格输出
- `TableStyle`：表格样式

---

## 🎯 设计模式

### 1. 命令模式

每个 stash 子命令都封装在独立的命令结构体中：
- `StashListCommand`
- `StashApplyCommand`
- `StashDropCommand`
- `StashPopCommand`
- `StashPushCommand`

### 2. 交互式设计

所有命令都提供交互式界面：
- 自动检测 stash 列表
- 提供友好的选择界面
- 显示清晰的确认信息

### 3. 错误处理

- 使用 `anyhow::Result` 统一错误处理
- 提供详细的错误消息
- 冲突检测和提示

---

## 🔍 错误处理

### 常见错误情况

1. **没有 stash 条目**
   - `stash list`：显示 "No stash entries found"
   - `stash apply/pop/drop`：显示警告并返回

2. **应用冲突**
   - 检测到冲突时，显示冲突提示
   - `stash pop` 在冲突时保留 stash 条目
   - 提供解决冲突的步骤提示

3. **删除失败**
   - 显示错误消息
   - 保留其他 stash 条目

### 错误消息示例

```
Warning: No stash entries available
Warning: Merge conflicts detected!
Warning: Please resolve conflicts manually:
  1. Resolve conflicts in the affected files
  2. Stage the resolved files with: git add <file>
  3. Continue with your workflow
```

---

## 📝 扩展性

### 添加新功能

1. **添加新的 stash 操作**：
   - 在 `src/lib/cli/stash.rs` 中添加新的子命令枚举
   - 在 `src/commands/stash/` 中创建新的命令文件
   - 在 `src/lib/git/stash.rs` 中添加底层实现（如需要）

2. **扩展输出格式**：
   - 可以添加 JSON/YAML 输出格式（参考 Jira 命令）
   - 使用 `OutputFormatArgs` 共用参数

3. **添加过滤选项**：
   - 可以添加按分支、时间等过滤 stash 的功能

---

## 📚 相关文档

- [Git 模块架构文档](../architecture/git.md) - Git 操作底层实现
- [CLI 架构文档](./cli.md) - CLI 命令结构设计
- [代码风格规范](../guidelines/development/code-style.md) - 代码风格规范
- [开发规范索引](../guidelines/development/README.md) - 开发规范总览

---

## 🎯 使用示例

### 列出所有 stash

```bash
# 列出所有 stash
workflow stash list

# 列出所有 stash 并显示统计信息
workflow stash list --stat
```

### 应用 stash

```bash
# 应用最新的 stash（交互式确认）
workflow stash apply

# 流程：
# 1. 显示最新 stash 信息
# 2. 询问是否应用
# 3. 如果选择其他，显示列表供选择
# 4. 应用 stash
# 5. 显示结果（成功/冲突）
```

### 删除 stash

```bash
# 删除 stash（交互式多选）
workflow stash drop

# 流程：
# 1. 显示所有 stash 列表
# 2. 多选要删除的 stash
# 3. 显示将要删除的列表
# 4. 确认删除
# 5. 执行删除
```

### 应用并删除 stash

```bash
# 应用并删除最新的 stash（交互式确认）
workflow stash pop

# 流程：
# 1. 显示最新 stash 信息
# 2. 询问是否应用
# 3. 如果选择其他，显示列表供选择
# 4. 应用并删除 stash
# 5. 如果有冲突，保留 stash 条目
# 6. 显示结果
```

---

**文档版本**：1.0
**最后更新**: 2025-01-27
