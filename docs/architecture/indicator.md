# Indicator 模块架构文档

## 📋 概述

本文档描述 Workflow CLI 的 Indicator 模块架构，包括：
- Spinner - 用于不确定进度的长时间运行操作
- Progress - 用于有明确进度的操作（如下载、上传等）

该模块提供统一的进度指示器功能，使用 `indicatif` 作为后端实现。提供简洁的 API，支持自动管理和手动管理两种使用方式。

**注意**：本模块是基础设施模块，被整个项目广泛使用。所有需要显示进度的长时间运行操作都使用这些指示器组件。

**模块统计：**
- 总代码行数：约 450+ 行
- 文件数量：3 个核心文件
- 主要组件：2 个指示器类型（Spinner, Progress）
- 依赖库：`indicatif`（进度条和 spinner）

---

## 📁 Lib 层架构（核心业务逻辑）

### 核心模块文件

```
src/lib/base/indicator/
├── mod.rs      # 模块声明和导出 (13行)
├── spinner.rs  # Spinner 实现 (196行)
└── progress.rs # Progress 实现 (258行)
```

### 依赖模块

- **`indicatif` crate**：提供进度条和 spinner 功能
- **`std::time::Duration`**：时间处理

### 模块集成

Indicator 模块被所有需要显示进度的命令广泛使用：

- **PR 命令**：使用 `Spinner` 显示创建 PR、同步 PR 等操作的进度
- **Jira 命令**：使用 `Spinner` 显示下载附件、清理日志等操作的进度
- **Lifecycle 命令**：使用 `Progress` 显示下载和安装进度
- **Log 命令**：使用 `Spinner` 显示下载日志、搜索日志等操作的进度
- **HTTP 客户端**：使用 `Progress` 显示下载进度

---

## 🔄 集成关系

Indicator 模块是 Workflow CLI 的基础设施模块，为所有需要显示进度的操作提供统一的进度指示功能。该模块通过以下方式与其他模块集成：

1. **命令层集成**：所有执行耗时操作的命令使用 `Spinner` 或 `Progress` 显示进度
2. **用户反馈**：提供清晰的进度反馈，改善用户体验
3. **自动化管理**：提供便捷方法自动管理指示器生命周期，简化使用

### 主要集成场景

- **PR 命令**：使用 `Spinner` 显示创建 PR、同步 PR 等操作的进度
- **Jira 命令**：使用 `Spinner` 显示下载附件、清理日志等操作的进度
- **Lifecycle 命令**：使用 `Progress` 显示下载和安装进度
- **Log 命令**：使用 `Spinner` 显示下载日志、搜索日志等操作的进度
- **HTTP 客户端**：使用 `Progress` 显示下载进度

---

## 🏗️ 架构设计

### 设计原则

1. **统一接口**：所有指示器使用相似的方法（`new()`, `finish()`, `update-_message()`）
2. **自动管理**：提供便捷方法自动管理指示器生命周期
3. **灵活使用**：支持手动管理和自动管理两种方式
4. **用户体验**：提供清晰的进度反馈

### 核心组件

#### 1. Spinner - 不确定进度指示器

用于显示长时间运行操作的进度指示器，适用于无法确定总进度的情况。

**主要方法**：
- `new(message)` - 创建新的 spinner 并立即开始显示
- `update-_message(message)` - 更新显示的消息
- `finish()` - 完成并清除 spinner
- `finish-_with-_message(message)` - 完成并显示完成消息
- `with(message, operation)` - 使用 spinner 执行一个操作（自动管理）
- `with-_output(message, operation)` - 使用 spinner 执行会产生输出的操作

**特性**：
- 自动开始显示
- 支持消息更新
- 支持自动管理（便捷方法）
- 支持处理子进程输出（`with-_output` 方法）

**使用示例**：

```rust
use workflow::base::indicator::Spinner;

// 方式 1：手动管理
let spinner = Spinner::new("Creating PR...");
// 执行耗时操作
spinner.finish();

// 方式 2：自动管理（推荐）
let result = Spinner::with("Creating PR...", || {
    provider.create-_pull-_request(...)
})?;

// 方式 3：处理会产生输出的操作
Spinner::with-_output("Pushing to remote...", || {
    GitBranch::push(&current-_branch, false)
})?;
log-_success!("Pushed to remote successfully");
```

**设计说明**：

- `with()` 方法：自动创建 spinner，执行操作，然后清理 spinner。适用于不产生输出的操作。
- `with-_output()` 方法：先显示 spinner 消息（250ms），然后完成 spinner，再执行操作。这样可以避免子进程的输出与 spinner 动画混合。适用于执行会产生 stdout/stderr 输出的操作（如 `git push`）。

#### 2. Progress - 确定进度指示器

用于显示有明确进度的操作（如下载文件、处理多个项目等）。

**主要方法**：
- `new(total, message)` - 创建一个新的进度条（已知总数）
- `new-_download(total-_bytes, message)` - 创建一个新的进度条（用于下载，显示字节数）
- `new-_unknown(message)` - 创建一个新的进度条（未知总数，使用 spinner 模式）
- `inc(delta)` - 增加进度（按单位数）
- `inc-_bytes(delta)` - 增加进度（按字节数）
- `set-_position(pos)` - 设置当前位置
- `update-_message(message)` - 更新显示的消息
- `finish()` - 完成并清除进度条
- `finish-_ref()` - 完成并清除进度条（不需要 move，用于 Mutex 中）
- `finish-_with-_message(message)` - 完成并显示完成消息

**特性**：
- 支持已知总数和未知总数两种模式
- 支持下载模式（显示字节数和速度）
- 支持进度更新
- 支持消息更新

**使用示例**：

```rust
use workflow::base::indicator::Progress;

// 方式 1：已知总数
let progress = Progress::new(100, "Downloading files...");
for i in 0..100 {
    // 处理项目
    progress.inc(1);
}
progress.finish();

// 方式 2：下载模式（显示字节数）
let progress = Progress::new-_download(1024 * 1024, "Downloading...");
progress.inc-_bytes(1024);
progress.finish();

// 方式 3：未知总数（使用 spinner 模式）
let progress = Progress::new-_unknown("Downloading...");
// 执行操作
progress.finish();
```

**进度条样式**：

已知总数模式：
```
⠋ [00:01] [████████████████████████████] 50/100 (50%) Downloading files...
```

下载模式：
```
⠋ [00:01] [████████████████████████████] 512KB/1MB (512KB/s, ETA: 1s) Downloading...
```

未知总数模式：
```
⠋ [00:01] Downloading...
```

### 设计模式

#### 自动管理设计

提供便捷方法自动管理指示器生命周期：

```rust
// Spinner 自动管理
let result = Spinner::with("Creating PR...", || {
    provider.create-_pull-_request(...)
})?;

// Progress 需要手动管理（因为需要更新进度）
let progress = Progress::new(100, "Processing...");
for item in items {
    process-_item(item);
    progress.inc(1);
}
progress.finish();
```

**优势**：
- 简化代码
- 自动清理资源
- 减少错误（不会忘记调用 `finish()`）

#### 错误处理策略

指示器本身不处理错误，错误由调用者处理：

```rust
let result = Spinner::with("Creating PR...", || {
    provider.create-_pull-_request(...)
})?;
// 如果操作失败，spinner 会自动清理
```

**优势**：
- 统一的错误处理方式
- 使用 `?` 操作符简化错误传播
- 自动清理资源

---

## 🔄 调用流程与数据流

### 整体架构流程

```
应用层（命令、模块）
  ↓
Indicator API（Spinner, Progress）
  ↓
indicatif 库（底层进度显示）
  ↓
终端输出
```

### 数据流

#### Spinner 流程

```
Spinner::new("message")
  ↓
立即开始显示动画
  ↓
执行操作（手动或通过 with()）
  ↓
finish() 或 finish-_with-_message()
  ↓
清除显示
```

#### Progress 流程

```
Progress::new(total, "message")
  ↓
开始显示进度条
  ↓
inc(delta) 或 set-_position(pos)  // 更新进度
  ↓
update-_message("new message")  // 可选：更新消息
  ↓
finish() 或 finish-_with-_message()
  ↓
清除显示
```

### 与其他模块的集成

Indicator 模块是基础设施模块，被整个项目广泛使用：

- **CLI 命令层**：所有需要显示进度的命令使用指示器
- **HTTP 客户端**：使用 `Progress` 显示下载进度
- **Git 操作**：使用 `Spinner` 显示 Git 操作进度
- **Jira 操作**：使用 `Spinner` 显示 Jira API 调用进度
- **PR 操作**：使用 `Spinner` 显示 PR 创建、同步等操作进度

**依赖关系**：

```
indicator (基础设施)
  ↓
所有业务模块（commands, lib/*）
```

---

## 📝 扩展性

### 添加新的指示器类型

1. 在 `indicator/` 目录下创建新的模块文件（如 `gauge.rs`）
2. 实现指示器结构体和相关方法
3. 在 `mod.rs` 中声明模块并重新导出
4. 在 `src/lib/base/mod.rs` 中添加到全局导出（如果需要）

### 自定义进度条样式

可以通过修改 `ProgressStyle` 来自定义进度条样式：

```rust
let pb = ProgressBar::new(total);
pb.set-_style(
    ProgressStyle::default-_bar()
        .template("{spinner:.green} [{elapsed-_precise}] [{wide-_bar:.cyan/blue}] {pos}/{len} ({percent}%) {msg}")
        .unwrap()
        .progress-_chars("#>-"),
);
```

---

## 📚 相关文档

- [总体架构文档](./architecture.md)
- [TOOLS 模块架构文档](./tools.md)
- [Dialog 模块架构文档](./dialog.md)

---

## 📋 使用示例

### Spinner 使用

```rust
use workflow::base::indicator::Spinner;

// 手动管理
let spinner = Spinner::new("Creating PR...");
// 执行操作
spinner.finish();

// 自动管理
let result = Spinner::with("Creating PR...", || {
    provider.create-_pull-_request(...)
})?;

// 处理会产生输出的操作
Spinner::with-_output("Pushing to remote...", || {
    GitBranch::push(&current-_branch, false)
})?;
log-_success!("Pushed to remote successfully");
```

### Progress 使用

```rust
use workflow::base::indicator::Progress;

// 已知总数
let progress = Progress::new(100, "Downloading files...");
for i in 0..100 {
    progress.inc(1);
}
progress.finish();

// 下载模式
let progress = Progress::new-_download(1024 * 1024, "Downloading...");
progress.inc-_bytes(1024);
progress.finish();

// 未知总数
let progress = Progress::new-_unknown("Downloading...");
progress.finish();
```

---

## ✅ 总结

Indicator 模块为整个项目提供统一的进度指示器功能：

1. **Spinner**：不确定进度指示器，支持自动管理和手动管理
2. **Progress**：确定进度指示器，支持已知总数、下载模式和未知总数模式

**设计优势**：
- ✅ **易用性**：简洁的 API，支持自动管理
- ✅ **灵活性**：支持手动管理和自动管理两种方式
- ✅ **用户体验**：提供清晰的进度反馈
- ✅ **资源管理**：自动清理资源，减少错误
- ✅ **适应性**：支持处理子进程输出（`with-_output` 方法）

---

**最后更新**: 2025-12-16
