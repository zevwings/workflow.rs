...
| | [测试辅助工具指南](./references/helpers.md) | CliCommandBuilder、TestDataGenerator、路径获取函数 |
...
详细介绍测试辅助工具的使用方法，包括 CliCommandBuilder、TestDataGenerator 和路径获取函数。

**关键内容**：
- CliCommandBuilder 基本使用和 API 参考
- TestDataGenerator 数据生成方法
- **路径获取函数**：`test_home_dir()`, `test_config_dir()`, `test_data_dir()`, `test_cache_dir()`
- 测试环境隔离和跨平台兼容性
- 与源代码行为一致性
- 最佳实践和使用示例
...
### Q: 什么时候应该使用 `test_home_dir()` 而不是 `dirs::home_dir()`？

A:
- **使用 `test_home_dir()`**：当测试需要支持测试环境隔离时
  - 测试需要访问主目录下的配置文件
  - 测试需要支持环境变量隔离
  - 测试需要跨平台兼容性

- **使用 `dirs::home_dir()`**：当测试需要访问真实的系统路径时
  - 需要从真实的全局配置复制（如 `git_config_guard.rs::copy_from_global()`）
  - 需要验证真实系统路径的行为
  - 需要访问用户的实际配置文件

详细说明请参考 [测试辅助工具指南 - 路径获取函数](./references/helpers.md#3-路径获取函数)。

### Q: `CliTestEnv` 和 `TestIsolation` 有什么区别？

A:
- **`TestIsolation`**：基础隔离环境，提供：
  - 独立的工作目录（使用绝对路径）
  - 环境变量隔离（EnvGuard）
  - 可选的 Git 配置隔离（GitConfigGuard）
  - 可选的 Mock 服务器

- **`CliTestEnv`**：基于 `TestIsolation` 的高级测试环境，提供：
  - `TestIsolation` 的所有功能
  - Git 仓库初始化（`init_git_repo()`）
  - 配置文件创建（`create_config()`, `create_home_config()`）
  - 文件管理（`create_file()`）
  - 项目路径和用户路径管理（`project_path()`, `home_path()`）

**选择建议**：
- 需要 Git 仓库操作或配置文件管理 → 使用 `CliTestEnv`
...
A:
- **使用统一的路径获取函数**：`test_home_dir()`, `test_config_dir()` 等函数自动处理平台差异
- **使用 `#[cfg]` 标记平台特定代码**：明确标记平台特定的测试和代码
...
- [平台差异分析](../../analysis/platform_differences_analysis.md)
- [统一使用 dirs 指南](../../analysis/unify_dirs_usage_guide.md)

### Q: 测试环境会自动清理吗？

A: 是的，测试环境使用 RAII 模式自动清理：
...
A:
- **临时目录**：继续使用 `std::env::temp_dir()` 或 `tempfile::tempdir()`
  - `dirs` crate 不提供临时目录函数
  - 临时目录通常不需要跨平台特殊处理

- **标准目录**：使用统一的路径获取函数
  - `test_home_dir()` - 主目录
  - `test_config_dir()` - 配置目录
  - `test_data_dir()` - 数据目录
  - `test_cache_dir()` - 缓存目录
...
### 2025-01-28
- **新增路径获取函数文档**：在 `helpers.md` 中添加路径获取函数章节，说明 `test_home_dir()` 等函数的使用
