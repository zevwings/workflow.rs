# cargo-bloat 使用指南

> 本文档介绍如何使用 `cargo-bloat` 工具分析 Rust 二进制文件大小，帮助优化项目体积。

---

## 📋 目录

- [概述](#-概述)
- [安装](#-安装)
- [基本使用](#-基本使用)
- [常用命令](#-常用命令)
- [输出解读](#-输出解读)
- [最佳实践](#-最佳实践)
- [常见问题](#-常见问题)
- [参考资源](#-参考资源)

---

## 📖 概述

`cargo-bloat` 是一个用于分析 Rust 编译后二进制文件大小的工具，可以帮助开发者：

- **识别体积占用**：找出哪些 crate 或函数占用了最多的空间
- **优化二进制大小**：定位可以优化的依赖和代码
- **依赖审查**：评估引入新依赖的体积成本
- **发布前检查**：确保最终二进制文件大小在合理范围内

### 适用场景

- CLI 工具开发（如本项目）
- 嵌入式系统开发
- 容器镜像优化
- 移动应用开发
- 任何需要控制二进制大小的场景

---

## 🔧 安装

### 方式 1：使用 cargo install（推荐）

```bash
cargo install cargo-bloat
```

### 方式 2：使用项目 Makefile（推荐）

本项目在 `Makefile` 中提供了便捷命令：

```bash
# 安装 cargo-bloat
make setup-bloat

# 使用 cargo-bloat 分析（会自动检查是否安装）
make bloat

# 带参数使用
make bloat ARGS="--crates --limit 10"
```

### 验证安装

```bash
cargo bloat --version
```

### ⚠️ 为什么不能作为 dev-dependencies？

`cargo-bloat` **不能**作为 `dev-dependencies` 添加到 `Cargo.toml` 中，因为：

1. **它是二进制工具，不是库**：`cargo-bloat` 是一个 cargo 子命令工具，提供可执行文件，不提供库接口
2. **dev-dependencies 用于库依赖**：`dev-dependencies` 用于测试、示例、基准测试等需要的**库**，不适用于二进制工具
3. **安装方式不同**：二进制工具需要通过 `cargo install` 全局安装，而不是作为项目依赖

**替代方案**：
- ✅ 使用 `cargo install cargo-bloat` 全局安装
- ✅ 使用项目 `Makefile` 中的 `make setup-bloat` 和 `make bloat` 命令
- ✅ 在 CI/CD 中安装（见下方最佳实践）

---

## 🚀 基本使用

### 1. 分析当前项目（Debug 模式）

```bash
cargo bloat
```

这会分析当前项目的 Debug 构建，显示各 crate 的大小信息。

### 2. 分析 Release 构建

```bash
cargo bloat --release
```

**推荐**：通常使用 Release 模式进行分析，因为这是最终发布的版本。

### 3. 按 crate 分析

```bash
cargo bloat --release --crates
```

显示每个 crate 占用的空间大小，按大小排序。

### 4. 按函数分析

```bash
cargo bloat --release --functions
```

显示每个函数占用的空间大小，帮助定位体积较大的函数。

---

## 📝 常用命令

### 基本分析命令

#### 分析所有 crate

```bash
cargo bloat --release --crates
```

**输出示例**：
```
 File  .text     Size Crate
 0.0%   0.0%  1.2KiB [Unknown]
 0.0%   0.0%  1.2KiB std
 0.0%   0.0%  1.2KiB workflow
 0.0%   0.0%  1.2KiB serde
 0.0%   0.0%  1.2KiB tokio
 ...
```

#### 分析所有函数

```bash
cargo bloat --release --functions
```

**输出示例**：
```
 File  .text     Size     Size Crate
 0.0%   0.0%  1.2KiB  1.2KiB workflow::main
 0.0%   0.0%  1.2KiB  1.2KiB workflow::commands::pr::execute
 ...
```

#### 分析特定 crate

```bash
cargo bloat --release --crates --filter serde
```

只显示包含 "serde" 的 crate。

#### 分析特定函数

```bash
cargo bloat --release --functions --filter main
```

只显示包含 "main" 的函数。

### 高级分析命令

#### 显示时间信息

```bash
cargo bloat --release --crates --time
```

显示编译时间信息，帮助识别编译慢的依赖。

#### 显示完整路径

```bash
cargo bloat --release --crates --full-fn
```

显示完整的函数路径，而不是简化的名称。

#### 限制显示数量

```bash
cargo bloat --release --crates --limit 10
```

只显示前 10 个最大的 crate。

#### 最小大小过滤

```bash
cargo bloat --release --crates --min-size 1000
```

只显示大小超过 1000 字节的 crate。

#### 树形结构显示

```bash
cargo bloat --release --crates --tree
```

以树形结构显示 crate 依赖关系。

### 输出格式选项

#### JSON 格式输出

```bash
cargo bloat --release --crates --json
```

输出 JSON 格式，便于程序化处理。

#### 详细输出

```bash
cargo bloat --release --crates --verbose
```

显示更详细的调试信息。

---

## 📊 输出解读

### 输出列说明

#### Crate 分析输出

```
 File  .text     Size Crate
```

- **File**：占整个二进制文件的百分比
- **.text**：占 .text 段的百分比（代码段）
- **Size**：实际大小（字节）
- **Crate**：crate 名称

#### 函数分析输出

```
 File  .text     Size     Size Crate
```

- **File**：占整个二进制文件的百分比
- **.text**：占 .text 段的百分比
- **Size**：函数大小（字节）
- **Size**：累计大小（包含子函数）
- **Crate**：函数所属的 crate

### 输出示例解读

#### Crate 分析输出示例

```bash
$ cargo bloat --release --crates --limit 5
 File  .text     Size Crate
 2.3%   2.3%  1.2MiB workflow
 1.8%   1.8%  950KiB tokio
 1.2%   1.2%  650KiB serde
 0.8%   0.8%  420KiB reqwest
 0.5%   0.5%  280KiB regex
```

**解读**：
- `workflow` 是主 crate，占用 1.2MB（2.3%）
- `tokio` 异步运行时占用 950KB（1.8%）
- `serde` 序列化库占用 650KB（1.2%）
- `reqwest` HTTP 客户端占用 420KB（0.8%）
- `regex` 正则表达式库占用 280KB（0.5%）

#### 函数分析输出示例（包含总结行）

```bash
$ cargo bloat --release --functions
 File  .text     Size     Size Crate
 0.5%   1.0%   50KiB   50KiB workflow::main
 0.3%   0.6%   30KiB   30KiB tokio::runtime::Runtime::new
 ...
48.7%  96.2%  6.7MiB                And 16633 smaller methods. Use -n N to show more.
50.7% 100.0%  7.0MiB                .text section size, the file size is 13.8MiB
```

**总结行解读**：

1. **第一行总结**：`48.7%  96.2%  6.7MiB                And 16633 smaller methods. Use -n N to show more.`
   - **48.7%**：已显示的函数占整个二进制文件的 48.7%
   - **96.2%**：已显示的函数占 .text 段的 96.2%
   - **6.7MiB**：已显示的函数总大小为 6.7MB
   - **And 16633 smaller methods**：还有 16633 个更小的函数未显示
   - **Use -n N to show more**：提示可以使用 `-n N` 参数显示更多函数

2. **第二行总结**：`50.7% 100.0%  7.0MiB                .text section size, the file size is 13.8MiB`
   - **50.7%**：.text 段占整个二进制文件的 50.7%
   - **100.0%**：.text 段占自身的 100%（这是 .text 段的总结行）
   - **7.0MiB**：.text 段的总大小为 7.0MB（代码段大小）
   - **the file size is 13.8MiB**：整个二进制文件大小为 13.8MB

**重要说明**：
- `.text` 段包含所有可执行代码
- 二进制文件总大小（13.8MB）包含：
  - `.text` 段（代码）：7.0MB（50.7%）
  - 其他段（数据、符号表、调试信息等）：约 6.8MB（49.3%）
- 如果使用 `strip` 或启用 `strip = true`，可以减小二进制文件大小（移除符号表和调试信息）

---

## 💡 最佳实践

### 1. 定期检查二进制大小

在以下时机运行 `cargo-bloat`：

- **添加新依赖后**：评估新依赖的体积成本
- **发布前**：确保二进制大小在预期范围内
- **性能优化时**：识别可以优化的依赖
- **定期审查**：作为代码审查的一部分

### 2. 使用 Release 模式分析

```bash
# ✅ 推荐：使用 Release 模式
cargo bloat --release --crates

# ❌ 不推荐：Debug 模式不反映实际大小
cargo bloat --crates
```

### 3. 关注大型依赖

重点关注占用空间大的依赖：

```bash
# 找出占用超过 100KB 的 crate
cargo bloat --release --crates --min-size 100000
```

### 4. 对比分析

在添加或移除依赖前后进行对比：

```bash
# 添加依赖前
cargo bloat --release --crates > before.txt

# 添加依赖后
cargo bloat --release --crates > after.txt

# 对比差异
diff before.txt after.txt
```

### 5. 优化策略

根据分析结果采取优化措施：

#### 移除未使用的依赖

```bash
# 使用 cargo-udeps 检查未使用的依赖
cargo install cargo-udeps
cargo udeps
```

#### 使用 feature flags

在 `Cargo.toml` 中只启用需要的 feature：

```toml
[dependencies]
serde = { version = "1.0", features = ["derive"] }  # 只启用需要的 feature
tokio = { version = "1.0", features = ["rt", "net"] }  # 避免启用所有 feature
```

#### 使用替代依赖

考虑使用更轻量的替代方案：

- `ureq` 替代 `reqwest`（同步 HTTP）
- `miniserde` 替代 `serde`（轻量序列化）
- `smol` 替代 `tokio`（轻量异步运行时）

#### 启用 LTO（Link Time Optimization）

在 `Cargo.toml` 中启用 LTO：

```toml
[profile.release]
lto = true
codegen-units = 1
```

#### 使用 strip 减少符号

```bash
# 编译后 strip
strip target/release/workflow
```

或在 `Cargo.toml` 中配置：

```toml
[profile.release]
strip = true
```

### 6. 集成到 CI/CD

在 CI 流程中检查二进制大小：

```yaml
# .github/workflows/ci.yml
- name: Check binary size
  run: |
    cargo bloat --release --crates --limit 10
    # 可以设置大小阈值，超过则失败
```

### 7. 文档化基线

在项目文档中记录二进制大小基线：

```markdown
## 二进制大小基线

- Release 模式：~5MB
- 主要依赖：
  - workflow: ~1.2MB
  - tokio: ~950KB
  - serde: ~650KB
```

---

## ❓ 常见问题

### Q1: 为什么 Debug 和 Release 模式的大小差异很大？

**A**: Debug 模式包含调试信息、未优化的代码和符号表，而 Release 模式经过优化并可能移除了调试信息。通常 Release 模式比 Debug 模式小 3-10 倍。

**建议**：始终使用 `--release` 标志分析最终发布版本。

### Q2: 如何找出某个依赖为什么这么大？

**A**: 使用函数级别分析：

```bash
# 分析特定 crate 的函数
cargo bloat --release --functions --filter serde
```

### Q3: 二进制大小突然增加，如何定位原因？

**A**: 使用对比分析：

```bash
# 1. 记录当前大小
cargo bloat --release --crates > current.txt

# 2. 检查 git 历史，找出最近添加的依赖
git log --oneline --all -- Cargo.toml

# 3. 分析新增依赖的大小
cargo bloat --release --crates --filter <new-dependency>
```

### Q4: 如何减少 tokio 的大小？

**A**: 只启用需要的 feature：

```toml
[dependencies]
tokio = { version = "1.0", features = ["rt", "net", "fs"] }  # 只启用需要的
# 避免使用 default features
tokio = { version = "1.0", default-features = false, features = ["rt"] }
```

### Q5: cargo-bloat 分析很慢怎么办？

**A**:

1. 使用 `--limit` 限制输出数量
2. 使用 `--min-size` 过滤小依赖
3. 只在需要时运行，不要频繁执行

### Q6: 如何分析特定二进制文件？

**A**: 如果项目有多个二进制文件：

```bash
# 分析特定二进制
cargo bloat --release --crates --bin workflow
```

### Q7: 输出中的 [Unknown] 是什么？

**A**: `[Unknown]` 表示无法确定来源的代码，通常是：
- 内联函数
- 编译器生成的代码
- 链接器添加的代码

这通常是正常的，可以忽略。

---

## 📚 参考资源

### 官方资源

- **GitHub 仓库**：https://github.com/RazrFalcon/cargo-bloat
- **crates.io**：https://crates.io/crates/cargo-bloat

### 相关工具

- **cargo-udeps**：检查未使用的依赖
  ```bash
  cargo install cargo-udeps
  cargo udeps
  ```

- **cargo-tree**：显示依赖树
  ```bash
  cargo tree
  ```

- **cargo-audit**：安全审计
  ```bash
  cargo install cargo-audit
  cargo audit
  ```

### 优化资源

- **Rust 二进制大小优化指南**：https://github.com/johnthagen/min-sized-rust
- **Cargo 文档 - 配置文件**：https://doc.rust-lang.org/cargo/reference/profiles.html

---

## 🔗 相关文档

- [开发规范指南](./DEVELOPMENT_GUIDELINES.md)
- [测试规范指南](./TESTING_GUIDELINES.md)
- [提交前检查指南](./workflows/PRE_COMMIT_GUIDELINES.md)

---

**最后更新**: 2024-12-19
