...
#[test]
fn test_platform_release_identifier_macos_intel() {
...
#[test]
fn test_platform_release_identifier_macos_apple_silicon() {
    let platform = Platform::new("macos", "aarch64");
    let identifier = platform.release_identifier().unwrap();
...
fn test_platform_release_identifier_linux_x86_64() {
    ...
    let identifier = platform.release_identifier().unwrap();
    // 可能是 Linux-x86_64 或 Linux-x86_64-static，取决于 is_static_required()
    assert!(identifier == "Linux-x86_64" || identifier == "Linux-x86_64-static");
}

#[test]
fn test_platform_release_identifier_linux_arm64() {
    let platform = Platform::new("linux", "aarch64");
    let identifier = platform.release_identifier().unwrap();
...
#[test]
fn test_platform_release_identifier_windows_arm64() {
...
fn test_platform_release_identifier_linux_x86_64_static_detection() {
    ...
    // 这个测试会触发 is_static_required() 的内部逻辑
    let platform = Platform::new("linux", "x86_64");
    let identifier = platform.release_identifier().unwrap();
...
#[test]
fn test_platform_is_static_required_non_linux() {
    // 测试非 Linux 平台不应该需要静态链接
    // 通过 release_identifier() 间接测试 is_static_required() 的第 118 行
    let macos_platform = Platform::new("macos", "x86_64");
    let identifier = macos_platform.release_identifier().unwrap();
    assert_eq!(identifier, "macOS-Intel");

    let windows_platform = Platform::new("windows", "x86_64");
    let identifier = windows_platform.release_identifier().unwrap();
    assert_eq!(identifier, "Windows-x86_64");
}

#[test]
fn test_platform_is_static_required_non_x86_64() {
    // 测试非 x86_64 架构不应该需要静态链接
    // 通过 release_identifier() 间接测试 is_static_required() 的第 118 行
    let linux_arm64_platform = Platform::new("linux", "aarch64");
    let identifier = linux_arm64_platform.release_identifier().unwrap();
    assert_eq!(identifier, "Linux-ARM64");
}
...
#[test]
fn test_platform_is_static_required_ldd_output_scenarios() {
    // 测试 ldd 命令的不同输出场景
    ...
    let macos_platform = Platform::new("macos", "x86_64");
    let macos_id = macos_platform.release_identifier().unwrap();
    assert_eq!(macos_id, "macOS-Intel");

    // 在非 x86_64 架构上，is_static_required 应该返回 false
    let linux_arm64_platform = Platform::new("linux", "aarch64");
    let linux_arm64_id = linux_arm64_platform.release_identifier().unwrap();
