...
#[test]
fn test_update_command_with_version_flag_parses_correctly() -> Result<()> {
    ...
    match cli.command {
        ...
        _ => return Err(color_eyre::eyre::eyre!("Expected Update command")),
    }
...
/// - version为指定的版本号（"1.2.3"）
#[test]
fn test_update_command_with_short_version_flag_parses_correctly() -> Result<()> {
...
/// - version为None（默认值）
#[test]
fn test_update_command_with_minimal_args_parses_correctly() -> Result<()> {
...
/// - 命令类型匹配预期
#[test]
fn test_lifecycle_commands_with_all_subcommands_parses_successfully() -> Result<()> {
...
/// - 错误消息明确指示不接受额外参数
#[test]
fn test_lifecycle_commands_with_extra_arguments_return_error() -> Result<()> {
    // Arrange: 准备所有命令和额外参数
