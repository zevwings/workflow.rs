...
fn test_create_preview_not_head() {
    with_test_repo(|_| {
        ...
        let first_commit_sha = String::from_utf8(first_commit_sha.stdout)
            .unwrap()
            .trim()
            .to_string();

        let commit_info = Command::new("git")
            .args(&["log", "-1", "--format=%H|%s|%an <%ae>|%ai", &first_commit_sha])
            .output()
...
fn test_format_preview_with_pushed_warning() {
    with_test_repo(|_| {
        ...
        // 创建预览信息（模拟已推送的情况）
        let mut preview = CommitReword::create_preview(
            &commit_info,
            "New message",
            true,
            &current_branch,
