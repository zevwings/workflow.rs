...
//!
//! #[test]
//! fn test_with_full_isolation_return_ok() -> color_eyre::Result<()> {
...
mod tests {
    ...
    #[test]
    fn test_test_isolation_basic_return_ok() -> Result<()> {
        let isolation = TestIsolation::new()?;
    ...
    /// - base_url不为空
    #[test]
    fn test_test_isolation_with_mock_server_return_ok() -> Result<()> {
        let isolation = TestIsolation::new()?.with_mock_server()?;
    ...
    /// - Drop时自动恢复
    #[test]
    fn test_test_isolation_env_guard_return_ok() -> Result<()> {
        let mut isolation = TestIsolation::new()?;
