...
use workflow::cli::{
    ...
    DocsCheckSubcommand, DocsSubcommand, HomebrewSubcommand, PerformanceSubcommand, PrSubcommand,
    TestsCheckSubcommand, TestsDocsSubcommand, TestsMetricsSubcommand, TestsReportSubcommand,
    TestsSubcommand, TestsTrendsSubcommand, VersionSubcommand,
};
...
fn main() -> Result<()> {
    ...
    match cli.command {
        DevSubcommand::Docs { command } => match command {
            DocsSubcommand::Check { target } => match target {
                DocsCheckSubcommand::Integrity {
                    architecture,
                    timestamps,
                    ci,
                } => {
            ...
            DocsSubcommand::Report { command } => match command {
        ...
        DevSubcommand::Tests { command } => match command {
            TestsSubcommand::Check { command } => match command {
                TestsCheckSubcommand::Coverage {
                    threshold,
                    ci,
                    output,
                    check_type,
                    ...
                    let cmd =
                        tests::TestsCoverageCheckCommand::new(threshold, ci, output, check_type);
                    cmd.check()?;
                }
            },
            TestsSubcommand::Docs { command } => match command {
                TestsDocsSubcommand::Check { ci } => {
                    let cmd = tests::TestsDocsCheckCommand::new(ci);
                    cmd.check()?;
                }
            },
            TestsSubcommand::Metrics { command } => match command {
                TestsMetricsSubcommand::Collect { report, output } => {
                    let cmd = tests::TestsMetricsCollectCommand::new(report, output);
                    cmd.collect()?;
                }
            },
            TestsSubcommand::Report { command } => match command {
                TestsReportSubcommand::Generate {
                    format,
                    output,
                    comment,
                    report: reports,
                    ...
                    let cmd =
                        tests::TestsReportGenerateCommand::new(format, output, comment, reports);
                    cmd.generate()?;
                }
            },
            TestsSubcommand::Trends { command } => match command {
                TestsTrendsSubcommand::Analyze {
                    metrics_dir,
                    output,
                } => {
        ...
        DevSubcommand::Performance { command } => match command {
            PerformanceSubcommand::Analyze {
                current,
                baseline,
                output,
                threshold,
            } => {
                let cmd = performance::PerformanceAnalyzeCommand::new(
                    current, baseline, output, threshold,
                );
                cmd.analyze()?;
            }
        },
        DevSubcommand::Version { command } => match command {
            VersionSubcommand::Generate { master, update, ci } => {
                let command = version::VersionGenerateCommand::new(master, update, ci);
                command.generate()?;
            }
        },
        DevSubcommand::Ci { command } => match command {
            CiSubcommand::CheckSkip {
                branch,
                pr_creator,
                expected_user,
                ci,
        ...
        DevSubcommand::Pr { command } => match command {
            PrSubcommand::Create {
                version,
                branch,
                base,
