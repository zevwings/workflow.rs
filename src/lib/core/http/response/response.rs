...
use crate::core::http::response::parser::{JsonParser, ResponseParser, TextParser};
...
impl HttpResponse {
    ...
    pub fn from_reqwest_response(
        ...
        let body_bytes = response.bytes().map_err(|e| {
            ...
            HttpResponseError::UnableToReadBody(format!("Unable to read response body: {}", e))
        })?;
        ...
        if body_bytes.len() > max_response_body_size {
            ...
            return Err(HttpResponseError::UnableToReadBody(format!(
    ...
    /// use workflow::http::HttpResponse;
    ///
    /// # fn main() -> Result<(), Box<dyn std::error::Error>> {
    /// # let client = workflow::http::HttpClient::global()?;
    /// # let url = "https://api.example.com";
    /// # let config = workflow::http::RequestConfig::new();
    ...
    pub fn ensure_success(self) -> Result<Self> {
        if !self.is_success() {
            ...
            return Err(HttpResponseError::ResponseFailed {
                status: self.status,
                body,
            }
            .into());
    ...
    /// ```rust,no_run
    /// use serde_json::Value;
    /// use workflow::http::{HttpClient, HttpResponse, RequestConfig};
    /// use workflow::error;
    ///
    /// # fn main() -> Result<(), Box<dyn std::error::Error>> {
...
mod tests {
    ...
    fn test_http_response_as_json_invalid() {
        ...
        let result: Result<serde_json::Value> = response.as_json();
    ...
    fn test_http_response_extract_error_message_empty() {
        ...
        assert!(error_msg == "" || error_msg == "null");
    }
    ...
    fn test_from_reqwest_response_success() {
        use crate::core::http::mock::HttpMockServer;
