...
use crate::core::prompt::dialog::selection::filter::FuzzyFilter;
use crate::core::prompt::dialog::selection::renderer::{OptionListRenderer, OptionRenderer};
use crate::core::prompt::dialog::{
...
impl<'a> OptionRenderer for MultiSelectOptionRenderer<'a> {
    fn render_option(
        ...
        option_text: &str,
        is_current: bool,
        theme: &crate::prompt::style::theme::Theme,
    ) -> String {
                ...
                Ok(Event::Key(KeyEvent {
                    ...
                    match code {
                        ...
                        KeyCode::Enter => {
                            ...
                            crate::core::prompt::dialog::selection::renderer::clear_and_display_result_with_search(
...
macro_rules! multiselect {
    ...
    ($fmt:literal, $arg:expr, $options:expr) => {
        $crate::prompt::MultiSelectBuilder::new(format!($fmt, $arg), $options)
    };
    // 格式化字符串 + 多个参数（2个或更多）+ 选项（用逗号分隔）
    ($fmt:literal, $arg1:expr, $arg2:expr, $($arg:expr),+ $(,)?, $options:expr) => {
        $crate::prompt::MultiSelectBuilder::new(format!($fmt, $arg1, $arg2, $($arg),+), $options)
    };
    // 简单字符串字面量 + 选项列表
