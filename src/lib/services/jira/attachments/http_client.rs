...
use crate::core::http::{Authorization, HttpClient, HttpMethod, RequestConfig};
...
impl AttachmentDownloader {
    ...
    pub fn download_file(url: &str, output_path: &Path) -> Result<()> {
        ...
        let auth = if !is_cloudfront {
            ...
            Some(Authorization::basic(email, api_token))
        } else {
            None
        };
        ...
        if !response.status().is_success() && is_cloudfront {
            ...
            // 重试，这次使用 Basic Auth
            let (email, api_token) = get_auth()?;
            let auth_retry = Authorization::basic(email, api_token);
            let mut config_with_auth =
