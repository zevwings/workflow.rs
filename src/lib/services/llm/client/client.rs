...
//! ## 配置提供者
//!
//! LLM 客户端通过 `LLMConfigProvider` trait 获取配置，实现了依赖倒置原则。
//! 默认情况下，`LLMClient::global()` 使用 `SettingsAdapter` 从配置文件读取设置。
//! 也可以通过 `LLMClient::with_config()` 使用自定义配置提供者（主要用于测试）。
...
use super::config::LLMConfigProvider;
...
/// LLM 客户端
///
/// 所有 LLM 提供商使用同一个客户端实现，通过配置提供者区分不同的提供商。
/// 所有配置（URL、API key、model、response_format）都从配置提供者动态获取。
///
/// 支持两种使用方式：
...
pub struct LLMClient {
    config: &'static dyn LLMConfigProvider,
}

impl LLMClient {
    /// 获取全局 LLMClient 单例
    ///
    /// 返回进程级别的 LLMClient 单例，使用默认配置适配器（从 Settings 读取配置）。
    ...
    ///
    /// ```rust,no_run
    /// use workflow::llm::{LLMClient, LLMRequestParams};
    ///
    ...
    /// # 返回
    ///
    /// 返回使用指定配置提供者的 `LLMClient` 实例。
    ///
    /// # 示例
    ///
    /// ```rust,no_run
    /// use workflow::llm::{LLMClient, LLMConfigProvider, LLMRequestParams};
    /// use workflow::infra::adapters::config::SettingsAdapter;
    ///
    /// # fn main() -> Result<(), Box<dyn std::error::Error>> {
    /// // 注意：with_config 需要 'static 生命周期，通常使用 global() 方法
    /// let client = LLMClient::global();
    /// let params = LLMRequestParams {
    ///     user_prompt: "What is Rust?".to_string(),
    ///     ..Default::default()
    ...
    /// # }
    /// ```
    pub fn with_config(config: &'static dyn LLMConfigProvider) -> Self {
        LLMClient { config }
    }
    ...
    pub fn call(&self, params: &LLMRequestParams) -> Result<String> {
        use crate::core::http::HttpClientConfig;
        ...
        crate::log_debug!("LLM url: {}", url);
        crate::log_debug!("LLM payload: {}", payload);
        crate::log_debug!("LLM headers: {:?}", headers);
        crate::log_debug!("LLM provider: {}", provider);
        ...
        let http_response = HttpResponse::from_reqwest_response(response, max_response_body_size)?;
        ...
        match provider.as_str() {
            ...
            "proxy" => {
                let base_url = self
                    ...
                    .wrap_err("LLM proxy URL is not configured")?;
                Ok(format!(
                    "{}/chat/completions",
    ...
    fn build_headers(&self) -> Result<HeaderMap> {
        ...
        let llm_key = self
            .config
            .get_current_provider_key()
            .ok_or_else(|| eyre!("LLM key is empty in settings"))?;
        if llm_key.is_empty() {
    ...
    fn build_model(&self) -> Result<String> {
        ...
        match provider.as_str() {
            ...
            "proxy" => self
                .config
                .get_current_provider_model()
                .ok_or_else(|| eyre!("Model is required for proxy provider")),
            _ => self
                .config
                .get_current_provider_model()
                .ok_or_else(|| eyre!("Model is required")),
        }
    }
