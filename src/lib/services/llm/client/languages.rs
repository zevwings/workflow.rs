...
///
/// ```rust,no_run
/// use workflow::llm::get_language_requirement;
///
/// let original = "You are a helpful assistant.";
/// let enhanced = get_language_requirement(original, None);
/// // 返回包含强化语言要求的 prompt（语言从默认配置读取）
/// ```
pub fn get_language_requirement(
    system_prompt: &str,
    config: Option<&dyn LLMConfigProvider>,
) -> String {
    // 从配置提供者读取语言设置，如果没有提供则使用默认值
    let language_code = if let Some(config_provider) = config {
        config_provider.get_language()
    } else {
        // 使用默认配置适配器
