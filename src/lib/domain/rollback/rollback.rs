...
use crate::{
    core::shell::paths::config_file, log_debug, log_info, log_warn, settings::paths::Paths,
    shell::detect::Detect, shell::reload::Reload,
};
    ...
    pub fn rollback(backup_info: &BackupInfo) -> Result<RollbackResult> {
        ...
        let (shell_reload_success, shell_config_file) = match Detect::shell() {
            Ok(shell) => {
                ...
                let config_file_result = config_file(&shell);
