...
use crate::core::shell::source::{
    add_source_for_shell, has_source_for_shell, remove_source_for_shell,
};
...
impl Completion {
    ...
    /// `log`（set、check）、`jira`（info、related、changelog、comments、attachments、clean）、`github`、`llm`、`branch`（ignore、create、rename、switch、sync、delete）、`repo`（setup、show、clean）等子命令。
    ...
    pub fn configure_shell_config(shell: &Shell) -> Result<CompletionConfigResult> {
        ...
        let result = match shell {
            Shell::Zsh | Shell::Bash => {
                ...
                let added =
                    add_source_for_shell(shell, source_pattern, Some("Workflow CLI completions"))
                        .wrap_err_with(|| {
            ...
            Shell::Fish | Shell::PowerShell | Shell::Elvish => {
                ...
                // 检查是否已配置
                let already_exists = has_source_for_shell(shell, workflow_source)?;

                if !already_exists {
                    // 为 completion 文件添加 source 语句
                    add_source_for_shell(shell, workflow_source, Some("Workflow CLI completions"))
                        .wrap_err_with(|| {
                            format!(
    ...
    pub fn remove_completion_config(shell: &Shell) -> Result<()> {
        match shell {
            Shell::Zsh | Shell::Bash => {
                ...
                let source_pattern = COMPLETIONS_SOURCE_PATH;
                let removed =
                    remove_source_for_shell(shell, source_pattern).wrap_err_with(|| {
                        format!(
    ...
    pub fn is_shell_configured(shell: &Shell) -> Result<(bool, PathBuf)> {
        let config_path = config_file(shell)?;
        ...
        let configured = match shell {
            Shell::Zsh | Shell::Bash => {
                ...
                let source_pattern = COMPLETIONS_SOURCE_PATH;
                has_source_for_shell(shell, source_pattern).unwrap_or(false)
            }
            Shell::Fish | Shell::PowerShell | Shell::Elvish => {
                ...
                has_source_for_shell(shell, workflow_source).unwrap_or(false)
            }
            _ => false,
        };
    ...
    fn remove_completion_block_from_config(shell: &Shell) -> Result<()> {
        ...
        // 移除 completion 文件的 source 语句
        let removed = remove_source_for_shell(shell, workflow_source).wrap_err_with(|| {
            format!(
    ...
    /// - `workflow` 命令及其所有子命令（包括 `pr`（create、merge、approve、comment、close、status、list、update、sync、rebase、pick、summarize）、`log`（set、check）、`jira`、`github`、`llm`、`branch`（ignore、create、rename、switch、sync、delete）、`repo`（setup、show、clean）等）
