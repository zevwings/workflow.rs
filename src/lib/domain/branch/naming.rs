...
use crate::config::template::{BranchTemplateVars, TemplateConfig, TemplateEngine};
use crate::repo::config::RepoConfig;
use crate::services::git::GitBranch;
use crate::services::llm::CreateGenerator;
use crate::services::llm::TranslateLLM;
        ...
        match TemplateConfig::load_branch_template(jira_type) {
            Ok(template_str) => {
                ...
                match engine.render_string(&template_str, &vars) {
                    Ok(rendered) => {
                        success!("Generated branch name using template: {}", rendered);
