...
mod tests {
    ...
    /// - JSON 字符串与预期一致
    #[test]
    fn test_branch_config_serialization_with_valid_config_serializes_to_json_return_ok(
    ) -> Result<()> {
    ...
    /// - 字段值与 JSON 中的值一致
    #[test]
    fn test_branch_config_deserialization_with_valid_json_deserializes_config_return_ok(
    ) -> Result<()> {
    ...
    /// - 空 ignore 列表不会被序列化
    #[test]
    fn test_branch_config_empty_ignore_list_return_collect() -> Result<()> {
        // Arrange: 准备测试空的忽略列表
        let config = BranchConfig {
