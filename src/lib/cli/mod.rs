...
pub fn extract_command_name(command: &Option<Commands>) -> Option<String> {
    ...
    match cmd {
        Commands::Proxy { subcommand, .. } => {
            ...
            parts.push(
                match subcommand {
                    ProxySubcommand::On => "on",
                    ProxySubcommand::Off => "off",
                    ProxySubcommand::Check => "check",
                }
                .to_string(),
            );
        }
...
mod tests {
    ...
    /// 测试提取带子命令的命令名
    #[test]
    fn test_extract_command_name_with_subcommand() {
        use crate::cli::ProxySubcommand;
        let command = Some(Commands::Proxy {
            subcommand: ProxySubcommand::On,
            temporary: false,
        });
        let result = extract_command_name(&command);
        assert_eq!(result, Some("proxy-on".to_string()));
    }
