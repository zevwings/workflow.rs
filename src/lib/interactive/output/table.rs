...
impl TableBuilder {
    ...
    ///
    /// ```rust,no_run
    /// use workflow::interactive::{TableBuilder, TableStyle};
    ///
...
///
/// ```rust,no_run
/// use workflow::interactive::Tabled;
...
macro_rules! impl_tabled {
    ($type:ty, [$($header:expr),+], |$self:ident| $body:expr) => {
        impl $crate::interactive::Tabled for $type {
            fn headers() -> Vec<String> {
...
impl TableBuilder {
    ...
    ///
    /// ```rust,no_run
    /// use workflow::interactive::{TableBuilder, Tabled, TableStyle};
    ///
    /// struct User {
    ///     name: String,
    ///     age: u32,
