...
use crate::interactive::dialog::error::Result;
use crate::interactive::dialog::filter::FuzzyFilter;
use crate::interactive::dialog::raw_mode::RawModeGuard;
use crate::interactive::dialog::renderer::{OptionListRenderer, OptionRenderer};
...
macro_rules! multiselect {
    ...
    ($fmt:literal, $arg:expr, $options:expr) => {
        $crate::interactive::MultiSelectBuilder::new(format!($fmt, $arg), $options)
    };
    // 格式化字符串 + 多个参数（2个或更多）+ 选项（用逗号分隔）
    ($fmt:literal, $arg1:expr, $arg2:expr, $($arg:expr),+ $(,)?, $options:expr) => {
        $crate::interactive::MultiSelectBuilder::new(format!($fmt, $arg1, $arg2, $($arg),+), $options)
    };
    // 简单字符串字面量 + 选项列表
    ($msg:literal, $options:expr) => {
        $crate::interactive::MultiSelectBuilder::new($msg, $options)
    };
    // 变量或其他表达式 + 选项列表
