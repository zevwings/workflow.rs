...
macro_rules! select {
    ...
    ($fmt:literal, $arg:expr, $options:expr) => {
        $crate::interactive::SelectBuilder::new(format!($fmt, $arg), $options)
    };
    // 格式化字符串 + 选项列表（无格式化参数）
    ($fmt:literal, $options:expr) => {
        $crate::interactive::SelectBuilder::new(format!($fmt), $options)
    };
    // 简单字符串字面量 + 选项列表
    ($msg:literal, $options:expr) => {
        $crate::interactive::SelectBuilder::new($msg, $options)
    };
    // 变量或其他表达式 + 选项列表
