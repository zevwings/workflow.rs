...
impl CompletionGenerator {
    ...
    /// - `workflow` 命令及其所有子命令（包括 `pr`（create、merge、approve、comment、close、status、list、update、sync、rebase、pick、summarize）、`log`（set、check）、`jira`（info、related、changelog、comments、attachments、clean）、`config`（show、validate、export、import）、`github`、`llm`、`proxy`、`branch`（clean、ignore、prefix）、`migrate`（cleanup）等）
    ///
    /// # 返回
    ///
