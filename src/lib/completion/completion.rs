...
impl Completion {
    ...
    /// 包括 `pr`（create、merge、approve、comment、close、status、list、update、integrate、summarize）、
    ...
    ///
    /// 返回 completion 文件列表：
    /// - `_workflow` / `workflow.bash`: 包含 `workflow` 命令及其所有子命令（包括 `pr`（create、merge、approve、comment、close、status、list、update、integrate、summarize）、`log`、`jira`、`github`、`llm`、`branch` 等）
    ...
    /// - `workflow` 命令及其所有子命令（包括 `pr`（create、merge、approve、comment、close、status、list、update、integrate、summarize）、`log`、`jira`、`github`、`llm`、`proxy`、`log-level`、`branch` 等）
