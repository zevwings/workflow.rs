...
mod tests {
    ...
    ///
    /// ## 测试目的
    /// 验证模板变量结构体在序列化为 JSON 时能够正确处理特殊字符（引号、尖括号、斜杠等）。
    ///
    /// ## 测试场景
    /// 1. 创建包含特殊字符的分支模板变量（引号、尖括号、斜杠）
    ...
    ///
    /// ## 测试目的
    /// 验证所有模板变量类型（BranchTemplateVars、CommitTemplateVars、PullRequestTemplateVars）在包含所有字段时能够正确序列化。
    ///
    /// ## 测试场景
    /// 1. 创建包含所有字段的分支模板变量
    /// 2. 创建包含所有字段的提交模板变量
    /// 3. 创建包含所有字段的 PR 模板变量（包括 change_types 数组）
    /// 4. 验证所有变量都能正确序列化为 JSON
    /// 5. 验证 JSON 中包含所有预期的字段和类型
    ///
    /// ## 预期结果
    /// - 所有变量类型都能正确序列化
    /// - JSON 中包含所有字段
    /// - 字段类型正确（字符串、布尔值、数组等）
    /// - change_types 数组包含正确数量的元素
    #[test]
    ...
    ///
    /// ## 测试目的
    /// 验证模板变量结构体（BranchTemplateVars、CommitTemplateVars）实现了 Clone trait，能够正确克隆实例。
    ///
    /// ## 测试场景
    /// 1. 创建分支模板变量和提交模板变量实例
    /// 2. 调用 clone() 方法克隆实例
    ...
    ///
    /// ## 测试目的
    /// 验证模板变量结构体实现了 Debug trait，能够通过 format!("{:?}", vars) 格式化输出调试信息。
    ///
    /// ## 测试场景
    /// 1. 创建分支模板变量和提交模板变量实例
    /// 2. 使用 Debug 格式化输出
    ...
    ///
    /// ## 预期结果
    /// - Debug 格式化输出包含结构体名称（BranchTemplateVars、CommitTemplateVars）
    /// - 输出包含字段值
    /// - 输出不为空
    #[test]
    fn test_vars_debug() {
