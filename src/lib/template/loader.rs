...
pub struct PullRequestsTemplates {
    /// Default PR template
    #[serde(default = "default_pull_request_template")]
    pub default: String,
}
...
pub fn load_template_config() -> Result<TemplateConfig> {
    // Try to load from project-level config first
    if let Ok(project_config) = load_project_template_config() {
        return Ok(project_config);
    }

    // Fall back to global config
    if let Ok(global_config) = load_global_template_config() {
...
fn load_project_template_config() -> Result<TemplateConfig> {
    ...
    if let Some(template_section) = value.get("template") {
        let config: TemplateConfig = toml::from_str(
            &toml::to_string(template_section).context("Failed to serialize template section")?,
        )
...
fn load_global_template_config() -> Result<TemplateConfig> {
...
pub fn load_branch_template_by_type(branch_type: Option<&str>) -> Result<String> {
    ...
    let template = match branch_type {
        ...
        Some("hotfix") => config.branch.hotfix.as_ref().unwrap_or(&config.branch.default),
        Some("refactoring") => config.branch.refactoring.as_ref().unwrap_or(&config.branch.default),
