...
/// assert_eq!(extract_github_repo_from_url("ssh://git@github.com/owner/repo.git").unwrap(), "owner/repo");
/// ```
pub fn extract_github_repo_from_url(url: &str) -> Result<String> {
    // 匹配 ssh:// 格式: ssh://git@github.com/owner/repo.git 或 ssh://git@github-xxx/owner/repo.git
    // 支持 SSH host 别名（如 ssh://git@github-brainim/owner/repo.git）
    let ssh_proto_re = Regex::new(r"ssh://git@github[^/]*/(.+?)(?:\.git)?/?$")
