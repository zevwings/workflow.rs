...
//! - Completion 目录路径
//!
//! 注意：Shell 配置文件路径功能已迁移到 `core::shell::paths::config_file()`。
//! 请使用 `crate::core::shell::paths::config_file(shell)` 来获取 shell 配置文件路径。
...
// Shell 相关功能已迁移到 core::shell::paths，不再需要 Shell 类型
use color_eyre::{
