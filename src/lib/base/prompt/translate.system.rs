...
mod tests {
    ...
    fn test_translate_system_prompt_with_valid_constant_returns_non_empty() {
        ...
        // Note: TRANSLATE_SYSTEM_PROMPT is a compile-time constant with non-empty content
        // The constant is verified to exist and contain content at compile time
        let _ = TRANSLATE_SYSTEM_PROMPT;
    }
    ...
    fn test_translate_system_prompt_contains_example_specification() {
        ...
        assert!(
            prompt_lower.contains(example_keywords[0])
                || prompt_lower.contains(example_keywords[1]),
