...
mod tests {
    use super::*;
    use crate::base::logger::console::{debug, error, info, success, warning};
    use crate::{trace_debug, trace_error, trace_info, trace_warn};
    ...
    fn test_log_level_round_trip() -> Result<()> {
        ...
        for level in levels {
            ...
            let parsed = level_str.parse::<LogLevel>().map_err(|e| {
                color_eyre::eyre::eyre!("LogLevel round trip should succeed: {}", e)
            })?;
