...
use crate::prompt::dialog::Validator;
...
pub struct FormField {
    ...
    pub nested_form: Option<crate::prompt::form::builder::FormBuilder>,
...
pub struct NestedFormField {
    ...
    pub prompt: String,
    /// 嵌套表单
    pub nested_form: crate::prompt::form::builder::FormBuilder,
    /// 输入完成后显示的 title（可选，字段级别）
    pub result_title: Option<String>,
    /// 条件函数（可选，基于前面字段的值决定是否执行）
    pub condition: Option<Condition>,
...
impl NestedFormField {
    ...
    pub fn new(
        ...
        nested_form: crate::prompt::form::builder::FormBuilder,
    ) -> Self {
        Self {
            key: key.into(),
            prompt: prompt.into(),
