    ...
    fn save_to_shell_config(env_vars: &HashMap<String, String>) -> Result<()> {
        ...
        if let Some(start_pos) = existing_content.find(marker_start) {
            if let Some(end_pos) = existing_content[start_pos..].find(marker_end) {
                let block_content = &existing_content[start_pos + marker_start.len()..start_pos + end_pos];
        ...
        config_block.push_str("# These environment variables will be loaded when you start a new shell\n\n");
        ...
        for (i, line) in lines.iter().enumerate() {
            ...
            if trimmed.starts_with("export ") {
                for key in keys {
                    ...
                    if trimmed.len() > 7 + key.len() {
                        let rest = &trimmed[7..]; // 跳过 "export "
                        if rest.starts_with(key) {
                            let after_key = &rest[key.len()..].trim_start();
