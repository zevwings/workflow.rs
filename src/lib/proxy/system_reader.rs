...
mod tests {
    use super::*;
    use crate::proxy::{ProxyConfig, ProxyConfigGenerator, ProxyInfo, ProxyType};
    ...
    fn test_proxy_info_get_proxy_url_with_enabled_proxies_returns_urls() -> Result<()> {
        ...
        let http_url =
            http_url.ok_or_else(|| color_eyre::eyre::eyre!("operation should succeed"))?;
        assert!(http_url.contains("http://"));
        assert!(http_url.contains("proxy.example.com"));
        assert!(http_url.contains("8080"));
        let https_url =
            https_url.ok_or_else(|| color_eyre::eyre::eyre!("operation should succeed"))?;
    ...
    ///
    /// ## 测试目的
    /// 验证 ProxyType 枚举的所有方法和功能，包括获取所有类型、环境变量键名和 URL 协议方案。
    ///
    /// ## 测试场景
    /// 1. 测试 ProxyType::all() 返回所有代理类型（Http、Https、Socks）
    /// 2. 测试 env_key() 方法返回正确的环境变量键名
    /// 3. 测试 url_scheme() 方法返回正确的 URL 协议方案
    /// 4. 测试 all_env_keys() 方法返回所有环境变量键名
    ///
    /// ## 预期结果
    /// - all() 返回3种代理类型
    /// - Http、Https、Socks 的环境变量键名分别为 "http_proxy"、"https_proxy"、"all_proxy"
    /// - Http 和 Https 的 URL 协议为 "http"，Socks 为 "socks5"
    /// - all_env_keys() 返回所有3个环境变量键名
    #[test]
    ...
    ///
    /// ## 测试目的
    /// 验证 ProxyInfo 能够处理混合配置（部分代理启用、部分禁用），并正确生成环境变量。
    ///
    /// ## 测试场景
    /// 1. 创建包含混合配置的 ProxyInfo（HTTP 启用、HTTPS 禁用、SOCKS 启用）
    /// 2. 使用 ProxyConfigGenerator::generate_env_vars() 生成环境变量
    /// 3. 验证只包含启用代理的环境变量
    /// 4. 验证 URL 格式正确（协议、地址、端口）
    ///
    /// ## 预期结果
    /// - 只生成启用代理的环境变量（http_proxy、all_proxy）
    /// - 不生成禁用代理的环境变量（https_proxy）
    /// - HTTP URL 格式为 "http://address:port"
    /// - SOCKS URL 格式为 "socks5://address:port"
    #[test]
    ...
    ///
    /// ## 测试目的
    /// 验证 ProxyInfo 和 ProxyConfigGenerator 能够正确处理无效配置和边界情况。
    ///
    /// ## 测试场景
    /// 1. 测试无效配置（空地址、无端口）
    /// 2. 验证无效配置不生成环境变量
    ...
    ///
    /// ## 测试目的
    /// 验证 ProxyInfo 支持动态添加、更新和删除配置，以及获取配置的可变引用。
    ///
    /// ## 测试场景
    /// 1. 创建空的 ProxyInfo 实例
    /// 2. 验证初始状态没有配置
    /// 3. 使用 set_config() 添加配置
    /// 4. 验证配置已添加
    /// 5. 使用 get_config_mut() 获取可变引用并更新配置
    /// 6. 验证配置已更新
    ///
    /// ## 预期结果
    /// - 初始状态没有配置（get_config 返回 None）
    /// - 添加配置后，get_config 返回正确的配置
    /// - 通过 get_config_mut() 更新配置后，配置值正确更新
    #[test]
