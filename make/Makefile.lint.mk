...
define HELP_LINT
	...
	@echo "  make check-links      - 检查文档中的链接有效性（使用 lychee）"
	@echo ""
endef
...
# 检查 lychee 是否安装
check-lychee:
	@command -v lychee >/dev/null 2>&1 || (echo "错误: lychee 未安装。运行 'make setup' 或 'cargo install lychee'" && exit 1)
...
# 检查文档中的链接有效性
check-links: check-lychee
	@echo "检查文档中的链接有效性..."
	@echo ""
	@lychee --verbose --exclude-all-private \
		--exclude ".*\{MODULE\}.*" \
		--exclude ".*\{TOPIC\}.*" \
		--exclude ".*\{VERSION\}.*" \
		--exclude ".*RELEASE_WORKFLOW_ANALYSIS\.md" \
		--exclude ".*docs/docs/.*" \
		docs/**/*.md || (echo "✗ 发现无效链接，请检查上述错误" && exit 1)
	@echo "✓ 所有链接检查通过！"
