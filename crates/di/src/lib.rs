...
//! ```
//!
//! ## 方式 2：使用 Container::global().try_bind()（显式类型转换）
//!
//! ```rust,ignore
//! Container::global().try_bind::<dyn ConfigService>(|c| {
//!     let dep = c.get::<dyn OtherService>()?;
//!     Ok(Arc::new(ConfigServiceImpl::new(dep)) as Arc<dyn ConfigService>)
//! })
//! .in_scope(Scope::Singleton)?;
//! ```
//!
...
//! Container::global().try_bind::<dyn ConfigService>(move |_| Ok(instance.clone()))
//!     .in_scope(Scope::Singleton)?;
//! ```
//!
    ...
    ($trait_type:ty, $factory:expr) => {{
        ...
        let wrapped: ::std::boxed::Box<
            dyn for<'a> ::std::ops::Fn(
                    ...
                    -> Result<::std::sync::Arc<$trait_type>, $crate::InjectionError>
                + ::std::marker::Send
                + ::std::marker::Sync,
        > = ::std::boxed::Box::new(
            move |c: &$crate::Container| -> Result<::std::sync::Arc<$trait_type>, $crate::InjectionError> {
...
/// ```
pub fn resolve<T: 'static + Send + Sync + ?Sized>() -> Result<Arc<T>, InjectionError> {
...
macro_rules! registry {
    (|$container:ident| { $($body:tt)* }) => {
        $crate::Container::register(|$container| -> Result<(), InjectionError> {
            $($body)*
            Ok(())
        })
    };
...
mod tests {
    ...
    fn test_resolve_success() {
        ...
        Container::global()
            .bind::<dyn TestService>(|_: &Container| {
                Ok(Arc::new(TestServiceImpl { value: 42 }) as Arc<dyn TestService>)
            })
            .in_scope(Scope::Singleton)
            .unwrap();
    ...
    fn test_resolve_not_bound() {
        ...
        let result: Result<Arc<dyn NonExistent>, InjectionError> = resolve();
    ...
    fn test_try_bind_macro_with_dependency() {
        ...
        Container::global()
            .bind::<dyn ConfigService>(|_: &Container| {
                Ok(Arc::new(ConfigServiceImpl { value: 42 }) as Arc<dyn ConfigService>)
            })
            .in_scope(Scope::Singleton)
            .unwrap();
    ...
    fn test_try_bind_macro_error() {
        ...
        bind!(dyn TestService, |_: &Container| {
            Err::<Arc<TestServiceImpl>, _>(InjectionError::NotBound("Simulated error".to_string()))
        })
        .in_scope(Scope::Singleton)
        .unwrap();

        let result: Result<Arc<dyn TestService>, InjectionError> = Container::global().get();
        assert!(result.is_err());
    }
    ...
    fn test_registry_macro_basic() {
        ...
        let result = registry!(|_container| {
            Container::global()
                .bind::<dyn TestService>(|_: &Container| {
                    Ok(Arc::new(TestServiceImpl { value: 500 }) as Arc<dyn TestService>)
                })
                .in_scope(Scope::Singleton)?;
        });
    ...
    fn test_registry_macro_error_propagation() {
        ...
        Container::global()
            .bind::<dyn TestService>(|_: &Container| {
                Ok(Arc::new(TestServiceImpl { value: 1 }) as Arc<dyn TestService>)
            })
            .in_scope(Scope::Singleton)
            .unwrap();
        ...
        let result = registry!(|_container| {
            Container::global()
                .bind::<dyn TestService>(|_: &Container| {
                    Ok(Arc::new(TestServiceImpl { value: 2 }) as Arc<dyn TestService>)
                })
                .in_scope(Scope::Singleton)?;
        });
