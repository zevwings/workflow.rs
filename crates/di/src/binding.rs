...
impl Binding {
    ...
    pub(crate) fn try_unbox_arc<T: 'static + Send + Sync + ?Sized>(
        ...
        if actual_type_id != expected_type_id {
            return Err(crate::error::InjectionError::TypeCast(format!(
        ...
        boxed.downcast_ref::<Arc<T>>().map(Arc::clone).ok_or_else(|| {
            crate::error::InjectionError::TypeCast(format!(
...
mod tests {
    ...
    fn test_fallible_binding_resolve_error() -> Result<()> {
        ...
        let factory: FallibleErasedFactory = Box::new(|_| {
            Err(crate::error::InjectionError::NotBound(
        ...
        assert!(matches!(
            result,
            Err(crate::error::InjectionError::NotBound(_))
        ));
    ...
    fn test_try_unbox_arc_type_mismatch() -> Result<()> {
        ...
        assert!(matches!(
            result,
            Err(crate::error::InjectionError::TypeCast(_))
        ));
