...
impl Container {
    ...
    pub fn get<T: 'static + Send + Sync + ?Sized>(&self) -> Result<Arc<T>> {
        ...
        RESOLUTION_STACK.with(|stack| {
            ...
            if stack.contains(&identifier) {
                ...
                let cycle: Vec<String> = stack[cycle_start..]
                    ...
                    .map(|id| format!("{:?}", id))
                    .collect();
                return Err(InjectionError::CircularDependency(cycle.join(" -> ")));
            }
        ...
        let result = if let Some(binding) = self.bindings.get(&identifier) {
            ...
            Err(InjectionError::NotBound(format!(
    ...
    pub(crate) fn add_binding(&self, binding: Binding) -> Result<()> {
        ...
        {
            return Err(InjectionError::AlreadyBound(
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if errors.is_empty() {
            ...
            Err(InjectionError::ValidationError(format!(
...
mod tests {
    ...
    fn test_register_error() -> Result<()> {
        ...
        assert!(matches!(result, Err(InjectionError::AlreadyBound(_))));
    ...
    fn test_get_not_bound() -> Result<()> {
        ...
        assert!(result.is_err());
        assert!(matches!(result, Err(InjectionError::NotBound(_))));
    ...
    fn test_validate_with_panicking_factory() -> Result<()> {
        ...
        if let Err(InjectionError::ValidationError(msg)) = result {
    ...
    fn test_multiple_services() -> Result<()> {
        ...
        container
            .bind::<dyn ServiceA>(|_: &Container| Ok(Arc::new(ServiceAImpl) as Arc<dyn ServiceA>))
            .in_scope(Scope::Singleton)?;

        container
            .bind::<dyn ServiceB>(|_: &Container| Ok(Arc::new(ServiceBImpl) as Arc<dyn ServiceB>))
            .in_scope(Scope::Singleton)?;
    ...
    fn test_bind_with_arc_instance() -> Result<()> {
        ...
        container
            .bind::<dyn TestService>(move |_: &Container| Ok(instance.clone()))
            .in_scope(Scope::Singleton)?;
    ...
    fn test_bind_concrete_type() -> Result<()> {
        ...
        container
            .bind::<TestServiceImpl>(|_: &Container| {
                Ok(Arc::new(TestServiceImpl { value: 777, id: 88 }))
            })
            .in_scope(Scope::Singleton)?;
    ...
    fn test_bind_with_dependencies() -> Result<()> {
        ...
        _ = container
            .bind::<dyn AppService>(|c: &Container| {
                ...
                Ok(Arc::new(AppServiceImpl { config }) as Arc<dyn AppService>)
            })
            .in_scope(Scope::Singleton);
    ...
    fn test_bind_instance_vs_bind_trait() -> Result<()> {
        ...
        container
            .bind::<dyn MyService>(move |_: &Container| Ok(trait_instance.clone()))
            .in_scope(Scope::Singleton)?;
    ...
    fn test_circular_dependency_direct() -> Result<()> {
        ...
        assert!(result.is_err());
        assert!(matches!(result, Err(InjectionError::CircularDependency(_))));
    ...
    fn test_try_bind_success() -> Result<()> {
        ...
        container
            .bind::<dyn TestService>(|_c: &Container| {
                Ok(Arc::new(TestServiceImpl { value: 42, id: 1 }) as Arc<dyn TestService>)
            })
            .in_scope(Scope::Singleton)?;
    ...
    fn test_try_bind_factory_returns_error() -> Result<()> {
        ...
        container
            .bind::<dyn TestService>(|_c: &Container| {
                Err(InjectionError::NotBound("Dependency not found".to_string()))
            })
            .in_scope(Scope::Singleton)?;
    ...
    fn test_validate_fallible_binding_error() -> Result<()> {
        ...
        container
            .bind::<dyn TestService>(|_: &Container| {
                Err(InjectionError::NotBound("Simulated error".to_string()))
            })
            .in_scope(Scope::Singleton)?;
            ...
            },
            {
                ...
                thread::spawn(move || {
                    c.bind::<dyn ServiceB>(|_: &Container| {
                        Ok(Arc::new(ServiceBImpl) as Arc<dyn ServiceB>)
                    })
                    .in_scope(Scope::Singleton)
                })
            },
            {
                ...
                thread::spawn(move || {
                    c.bind::<dyn ServiceC>(|_: &Container| {
                        Ok(Arc::new(ServiceCImpl) as Arc<dyn ServiceC>)
                    })
                    .in_scope(Scope::Singleton)
                })
            },
            {
                ...
                thread::spawn(move || {
                    c.bind::<dyn ServiceD>(|_: &Container| {
                        Ok(Arc::new(ServiceDImpl) as Arc<dyn ServiceD>)
                    })
                    .in_scope(Scope::Singleton)
                })
            },
    ...
    fn test_singleton_rebind_after_unbind() -> Result<()> {
        ...
        container
            .bind::<dyn TestService>(|_: &Container| {
                Ok(Arc::new(TestServiceImpl { value: 100, id: 1 }) as Arc<dyn TestService>)
            })
            .in_scope(Scope::Singleton)?;
        ...
        container
            .bind::<dyn TestService>(|_: &Container| {
                Ok(Arc::new(TestServiceImpl { value: 200, id: 2 }) as Arc<dyn TestService>)
            })
            .in_scope(Scope::Singleton)?;
    ...
    fn test_unbind_all_and_rebind() -> Result<()> {
        ...
        container
            .bind::<dyn TestService>(|_: &Container| {
                Ok(Arc::new(TestServiceImpl { value: 1, id: 1 }) as Arc<dyn TestService>)
            })
            .in_scope(Scope::Singleton)?;

        container
            .bind::<dyn ServiceA>(|_: &Container| {
                Ok(Arc::new(ServiceAImpl { name: "first" }) as Arc<dyn ServiceA>)
            })
            .in_scope(Scope::Singleton)?;
        ...
        container
            .bind::<dyn ServiceA>(|_: &Container| {
                Ok(Arc::new(ServiceAImpl { name: "second" }) as Arc<dyn ServiceA>)
            })
            .in_scope(Scope::Singleton)?;
    ...
    fn test_transient_to_singleton_rebind() -> Result<()> {
        ...
        container
            .bind::<dyn TestService>(move |_: &Container| {
                ...
                Ok(Arc::new(TestServiceImpl { value: 2, id: 2 }) as Arc<dyn TestService>)
            })
            .in_scope(Scope::Singleton)?;
    ...
    fn test_fallible_binding_error_not_cached() -> Result<()> {
        ...
        container
            .bind::<dyn TestService>(move |_: &Container| {
                let count = call_count_clone.fetch_add(1, Ordering::SeqCst);
                if count < 2 {
                    Err(InjectionError::NotBound("Simulated failure".to_string()))
                } else {
                    Ok(Arc::new(TestServiceImpl { value: 42, id: 1 }) as Arc<dyn TestService>)
                }
            })
