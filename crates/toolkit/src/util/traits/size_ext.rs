...
mod tests {
    ...
    fn test_to_size_string_basic(#[case] bytes: u64, #[case] expected: &str) {
        assert_eq!(bytes.to_size_string(), expected);
    }
    ...
    fn test_to_size_string_precision(#[case] bytes: u64, #[case] expected: &str) {
        assert_eq!(bytes.to_size_string(), expected);
    }

    #[test]
    fn test_to_size_string_edge_cases() {
        // 测试边界值
        assert_eq!(
            u64::MAX.to_size_string(),
            format!("{:.2} TB", u64::MAX as f64 / 1024_f64.powi(4))
        );

        // 测试刚好达到下一个单位的值
        assert_eq!((1024 - 1).to_size_string(), "1023 B");
        assert_eq!(1024.to_size_string(), "1.00 KB");
        assert_eq!((1024 * 1024 - 1).to_size_string(), "1024.00 KB");
        assert_eq!((1024 * 1024).to_size_string(), "1.00 MB");
    }
}
