...
impl FileReader {
    ...
    pub fn open(&self) -> Result<BufReader<File>, FsError> {
        let file = File::open(&self.path).map_err(FsError::Io)?;
        Ok(BufReader::new(file))
    }

    /// 读取文件内容为字符串。
    pub fn to_string(&self) -> Result<String, FsError> {
        fs::read_to_string(&self.path).map_err(FsError::Io)
    }

    /// 读取文件的所有行。
    pub fn lines(&self) -> Result<Vec<String>, FsError> {
        let file = File::open(&self.path).map_err(FsError::Io)?;
    ...
    pub fn bytes(&self) -> Result<Vec<u8>, FsError> {
        ...
        file.read_to_end(&mut buffer).map_err(FsError::Io)?;
        Ok(buffer)
    }

    /// 读取 TOML 文件并解析为类型 `T`。
    pub fn toml<T>(&self) -> Result<T, FsError>
    where
        T: DeserializeOwned,
    {
        let content = fs::read_to_string(&self.path).map_err(FsError::Io)?;
        toml::from_str(&content).map_err(FsError::Toml)
    }

    /// 读取 JSON 文件并解析为类型 `T`。
    pub fn json<T>(&self) -> Result<T, FsError>
    where
        T: DeserializeOwned,
    {
        let content = fs::read_to_string(&self.path).map_err(FsError::Io)?;
        serde_json::from_str(&content).map_err(FsError::Json)
    }
}
...
impl FileWriter {
    ...
    pub fn ensure_parent_dir(&self) -> Result<(), FsError> {
    ...
    #[cfg(unix)]
    pub fn set_permissions(&self, mode: u32) -> Result<(), FsError> {
        fs::set_permissions(&self.path, fs::Permissions::from_mode(mode)).map_err(FsError::Io)
    }

    /// 将字符串内容写入文件。
    pub fn write_str(&self, content: &str) -> Result<(), FsError> {
        fs::write(&self.path, content).map_err(FsError::Io)
    }
    ...
    pub fn write_str_with_dir(&self, content: &str) -> Result<(), FsError> {
        self.ensure_parent_dir()?;
        self.write_str(content)
    }

    /// 将字节内容写入文件。
    pub fn write_bytes(&self, content: &[u8]) -> Result<(), FsError> {
        fs::write(&self.path, content).map_err(FsError::Io)
    }
    ...
    ///
    /// 如果成功，返回 `Ok(())`；如果失败，返回错误。
    pub fn write_bytes_with_dir(&self, content: &[u8]) -> Result<(), FsError> {
        self.ensure_parent_dir()?;
        self.write_bytes(content)
    }

    /// 将类型 `T` 序列化为 TOML 并写入文件。
    pub fn write_toml<T>(&self, data: &T) -> Result<(), FsError>
    where
        T: Serialize,
    {
        let toml_content = toml::to_string_pretty(data)
            .map_err(|e| FsError::Other(format!("Failed to serialize to TOML: {}", e)))?;
    ...
    ///
    /// 如果成功，返回 `Ok(())`；如果失败，返回错误。
    pub fn write_toml_secure<T>(&self, data: &T) -> Result<(), FsError>
    where
        T: Serialize,
    {
        self.ensure_parent_dir()?;
    ...
    pub fn write_json<T>(&self, data: &T) -> Result<(), FsError>
    where
        T: Serialize,
    {
        let json_content = serde_json::to_string_pretty(data).map_err(FsError::Json)?;
    ...
    ///
    /// 如果成功，返回 `Ok(())`；如果失败，返回错误。
    pub fn write_json_secure<T>(&self, data: &T) -> Result<(), FsError>
    where
        T: Serialize,
    {
        self.ensure_parent_dir()?;
...
mod tests {
    ...
    mod reader_tests {
        ...
        fn test_file_reader_new() {
            ...
            let reader1 = FileReader::new(&test_path);
        ...
        fn test_bytes(temp_dir: TempDir, #[case] content: &[u8]) -> Result<(), FsError> {
        ...
        #[rstest]
        fn test_toml(temp_dir: TempDir) -> Result<(), FsError> {
...
"#;
        ...
        #[rstest]
        fn test_json(temp_dir: TempDir) -> Result<(), FsError> {
...
"#;
    ...
    mod writer_tests {
        ...
        fn test_file_writer_new() {
            // 测试使用 &str
            let test_path = std::env::temp_dir().join("test.txt");
            let writer1 = FileWriter::new(&test_path);
        ...
        #[test]
        fn test_ensure_parent_dir_for_root_path() -> Result<(), FsError> {
        ...
        #[cfg(unix)]
        #[test]
        fn test_set_permissions() -> Result<(), FsError> {
            let temp_dir = tempfile::tempdir()?;
            let file_path = temp_dir.path().join("test.txt");
            fs::File::create(&file_path)?;
        ...
        #[case("")]
        #[case("Hello, 世界!\n测试文件\némoji🚀")]
        fn test_write_str(temp_dir: TempDir, #[case] content: &str) -> Result<(), FsError> {
            let file_path = temp_dir.path().join("test.txt");
        ...
        #[rstest]
        fn test_write_str_with_dir(temp_dir: TempDir) -> Result<(), FsError> {
        ...
        fn test_write_bytes(temp_dir: TempDir, #[case] content: &[u8]) -> Result<(), FsError> {
            let file_path = temp_dir.path().join("test.bin");
        ...
        #[rstest]
        fn test_write_bytes_with_dir(temp_dir: TempDir) -> Result<(), FsError> {
        ...
        #[rstest]
        fn test_write_toml(temp_dir: TempDir) -> Result<(), FsError> {
            let file_path = temp_dir.path().join("config.toml");
        ...
        #[rstest]
        fn test_write_toml_secure(temp_dir: TempDir) -> Result<(), FsError> {
        ...
        #[rstest]
        fn test_write_toml_nested_structure(temp_dir: TempDir) -> Result<(), FsError> {
            #[derive(Serialize)]
            struct NestedConfig {
                app: TestConfig,
        ...
        #[rstest]
        fn test_write_json(temp_dir: TempDir) -> Result<(), FsError> {
            let file_path = temp_dir.path().join("config.json");
            let config = TestConfig {
                name: "test".to_string(),
                version: "1.0.0".to_string(),
        ...
        #[rstest]
        fn test_write_json_secure(temp_dir: TempDir) -> Result<(), FsError> {
        ...
        #[rstest]
        fn test_write_json_nested_structure(temp_dir: TempDir) -> Result<(), FsError> {
            #[derive(Serialize)]
            struct NestedConfig {
                app: TestConfig,
                database: DatabaseConfig,
        ...
        #[case("write_str")]
        #[case("write_bytes")]
        fn test_write_nonexistent_parent(#[case] method: &str) -> Result<(), FsError> {
