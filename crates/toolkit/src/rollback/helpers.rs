//! Completion 辅助函数
//!
//! 提供 completion 文件命名和列表相关的工具函数。

use thiserror::Error;

/// Completion 辅助函数错误类型
#[derive(Debug, Error)]
pub enum CompletionHelperError {
    /// 不支持的 Shell 类型
    #[error("Unsupported shell type: {0}. Supported shell types: zsh, bash, fish, powershell, elvish")]
    UnsupportedShell(String),
}

/// 根据 shell 类型和命令名生成补全脚本文件名
///
/// # 参数
///
/// * `shell_type` - Shell 类型（"zsh", "bash", "fish", "powershell", "elvish"）
/// * `command` - 命令名（"workflow"）
///
/// # 返回
///
/// 返回补全脚本文件名。
///
/// # 示例
///
/// ```
/// use toolkit::rollback::get_completion_filename;
///
/// assert_eq!(get_completion_filename("zsh", "workflow").unwrap(), "_workflow");
/// assert_eq!(get_completion_filename("bash", "workflow").unwrap(), "workflow.bash");
/// assert_eq!(get_completion_filename("fish", "workflow").unwrap(), "workflow.fish");
/// ```
pub fn get_completion_filename(
    shell_type: &str,
    command: &str,
) -> Result<String, CompletionHelperError> {
    match shell_type {
        "zsh" => Ok(format!("_{}", command)),
        "bash" => Ok(format!("{}.bash", command)),
        "fish" => Ok(format!("{}.fish", command)),
        "powershell" => Ok(format!("_{}.ps1", command)),
        "elvish" => Ok(format!("{}.elv", command)),
        _ => Err(CompletionHelperError::UnsupportedShell(shell_type.to_string())),
    }
}

/// 获取指定 shell 类型的所有补全脚本文件名
///
/// # 参数
///
/// * `shell_type` - Shell 类型
/// * `commands` - 命令名列表（通常为 ["workflow"]）
///
/// # 返回
///
/// 返回该 shell 类型的所有补全脚本文件名列表。
///
/// # 示例
///
/// ```
/// use toolkit::rollback::get_completion_files_for_shell;
///
/// let files = get_completion_files_for_shell("zsh", &["workflow"]).unwrap();
/// assert_eq!(files, vec!["_workflow"]);
/// ```
pub fn get_completion_files_for_shell(
    shell_type: &str,
    commands: &[&str],
) -> Result<Vec<String>, CompletionHelperError> {
    commands
        .iter()
        .map(|cmd| get_completion_filename(shell_type, cmd))
        .collect()
}

/// 获取所有 shell 类型的所有补全脚本文件名
///
/// 用于备份、删除等需要处理所有 shell 类型文件的场景。
///
/// # 参数
///
/// * `commands` - 命令名列表（通常为 ["workflow"]）
///
/// # 返回
///
/// 返回所有 shell 类型的所有补全脚本文件名列表。
///
/// # 示例
///
/// ```
/// use toolkit::rollback::get_all_completion_files;
///
/// let all_files = get_all_completion_files(&["workflow"]);
/// // 包含 zsh, bash, fish, powershell, elvish 的所有文件
/// assert!(all_files.contains(&"_workflow".to_string()));
/// assert!(all_files.contains(&"workflow.bash".to_string()));
/// assert!(all_files.contains(&"workflow.fish".to_string()));
/// ```
pub fn get_all_completion_files(commands: &[&str]) -> Vec<String> {
    let shell_types = ["zsh", "bash", "fish", "powershell", "elvish"];
    let mut all_files = Vec::new();

    for shell_type in &shell_types {
        if let Ok(files) = get_completion_files_for_shell(shell_type, commands) {
            all_files.extend(files);
        }
    }

    all_files
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_get_completion_filename() {
        assert_eq!(get_completion_filename("zsh", "workflow").unwrap(), "_workflow");
        assert_eq!(
            get_completion_filename("bash", "workflow").unwrap(),
            "workflow.bash"
        );
        assert_eq!(
            get_completion_filename("fish", "workflow").unwrap(),
            "workflow.fish"
        );
        assert_eq!(
            get_completion_filename("powershell", "workflow").unwrap(),
            "_workflow.ps1"
        );
        assert_eq!(
            get_completion_filename("elvish", "workflow").unwrap(),
            "workflow.elv"
        );
    }

    #[test]
    fn test_get_completion_filename_unsupported() {
        let result = get_completion_filename("unknown", "workflow");
        assert!(result.is_err());
    }

    #[test]
    fn test_get_all_completion_files() {
        let files = get_all_completion_files(&["workflow"]);
        assert_eq!(files.len(), 5);
        assert!(files.contains(&"_workflow".to_string()));
        assert!(files.contains(&"workflow.bash".to_string()));
        assert!(files.contains(&"workflow.fish".to_string()));
        assert!(files.contains(&"_workflow.ps1".to_string()));
        assert!(files.contains(&"workflow.elv".to_string()));
    }
}
