//! CNB 仓储实现
//!
//! 实现 `CNBRepository` trait，通过内部服务层协调各个组件。
//! Repository 层作为极薄的委托层，所有业务逻辑都在 services 层。

use std::sync::Arc;

use domain::{CNBError, CNBRepository, CNBUser, PullRequestInfo, PullRequestStatus};
...
use crate::cnb::{
    ...
    types::PullRequestInfo as CNBPullRequestInfo,
};

/// CNB 仓储实现
///
/// 实现 `CNBRepository` trait，通过依赖注入使用内部服务层。
pub struct CNBRepositoryImpl {
...
impl CNBRepositoryImpl {
    ...
    /// 验证 PR ID 不为空
    fn validate_pr_id(&self, pr_id: &str) -> Result<(), CNBError> {
        if pr_id.is_empty() {
            return Err(CNBError::ApiError("PR ID cannot be empty".to_string()));
        }
        Ok(())
    }

    /// 将 CNB API 的 PullRequestInfo 转换为 domain 的 PullRequestInfo
    fn convert_pr_info(pr_info: CNBPullRequestInfo) -> PullRequestInfo {
...
impl CNBRepository for CNBRepositoryImpl {
    fn create_pull_request(
        ...
        if title.is_empty() {
            return Err(CNBError::ApiError("PR title cannot be empty".to_string()));
        }
        ...
        // 从 URL 中提取 PR ID（例如：https://cnb.cool/owner/project/pulls/123 -> 123）
        let pr_id = pr_url
            .rsplit('/')
            .next()
            .ok_or_else(|| {
                CNBError::ApiError(format!("Failed to extract PR ID from URL: {}", pr_url))
            })?
            .to_string();
    ...
    fn get_pull_request(&self, pr_id: &str) -> Result<PullRequestInfo, CNBError> {
        ...
        let pr_info = self.query_service.fetch_pr_info(pr_id)?;
    ...
    fn add_comment(&self, pr_id: &str, comment: &str) -> Result<(), CNBError> {
        self.validate_pr_id(pr_id)?;
        if comment.is_empty() {
            return Err(CNBError::ApiError("Comment cannot be empty".to_string()));
        }
