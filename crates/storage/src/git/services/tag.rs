...
mod tests {
    ...
    fn test_create_annotated_tag() {
        with_isolated_git_env(|| {
            let (_tmp, ctx) = setup_repo_with_file();
            let service = TagServiceImpl::new(ctx.clone());
            ...
            let repo = ctx.repository();
            let reference = repo.find_reference("refs/tags/v1.2.0").unwrap();
            let tag = reference.peel_to_tag().unwrap();
            assert_eq!(tag.name().unwrap(), "v1.2.0");
        });
    }
    ...
    fn test_delete_tags_by_pattern() {
        with_isolated_git_env(|| {
...

            {
                ...
                let obj = repo.revparse_single("HEAD").unwrap();
                repo.tag_lightweight("v1.0.0", &obj, false).unwrap();
                repo.tag_lightweight("v1.1.0", &obj, false).unwrap();
                repo.tag_lightweight("v2.0.0", &obj, false).unwrap();
            }

            let results =
                service.delete_tags_by_pattern("v1.*", TagDeleteScope::Local, false).unwrap();
            assert_eq!(results.len(), 2);

            assert!(!service.tag_exists_local("v1.0.0"));
            assert!(!service.tag_exists_local("v1.1.0"));
            assert!(service.tag_exists_local("v2.0.0"));
        });
    }
    ...
    fn test_preview_delete_and_has_tag() {
        with_isolated_git_env(|| {
...

            {
                let repo = ctx.repository();
                let obj = repo.revparse_single("HEAD").unwrap();
                repo.tag_lightweight("preview-1", &obj, false).unwrap();
                repo.tag_lightweight("preview-2", &obj, false).unwrap();
            }

            let preview =
                service.preview_delete(None, Some("preview-*"), TagDeleteScope::Local).unwrap();
            assert_eq!(preview.len(), 2);

            let (local, remote) = service.has_tag("preview-1").unwrap();
            assert!(local);
            assert!(!remote);
        });
    }
