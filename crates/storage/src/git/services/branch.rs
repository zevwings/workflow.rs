...
mod tests {
    ...
    fn test_delete_local_branch() {
        with_isolated_git_env(|| {
            ...
            service.create_branch("temp-branch").unwrap();
            service.delete_local_branch("temp-branch", false).unwrap();

            let (exists, _) = service.has_branch("temp-branch").unwrap();
            assert!(!exists);
        });
    }
    ...
    fn test_delete_current_branch_fails() {
        with_isolated_git_env(|| {
            ...
            let current = service.get_current_branch().unwrap();
            let result = service.delete_local_branch(&current, true);
    ...
    fn test_rename_branch() {
        with_isolated_git_env(|| {
            ...
            service.create_branch("old-name").unwrap();
            service.rename_branch(Some("old-name"), "new-name").unwrap();

            let (old_exists, _) = service.has_branch("old-name").unwrap();
            let (new_exists, _) = service.has_branch("new-name").unwrap();
            assert!(!old_exists);
            assert!(new_exists);
        });
    }
    ...
    fn test_list_branches_remove_prefix() {
        with_isolated_git_env(|| {
            ...
            service.create_branch("feature/test").unwrap();

            let branches = service.list_branches(true, false).unwrap();
    ...
    fn test_checkout_branch() {
        with_isolated_git_env(|| {
            ...
            service.create_branch("feature/checkout").unwrap();
            service.checkout_branch("feature/checkout").unwrap();
    ...
    fn test_get_default_branch_from_remote_head() {
        with_isolated_git_env(|| {
            let (_tmp, ctx) = setup_repo();
            let service = BranchServiceImpl::new(ctx.clone());
            ...
            let default_branch = service.get_default_branch().unwrap();
    ...
    fn test_infer_target_branch_from_merge_base() {
        with_isolated_git_env(|| {
            ...
            service.create_branch("develop").unwrap();
            service.checkout_branch("develop").unwrap();
            service.create_branch("feature/one").unwrap();

            let target = service.infer_target_branch("feature/one").unwrap();
