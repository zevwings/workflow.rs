...
mod tests {
    ...
    fn test_open_nonexistent() {
        ...
        assert!(matches!(result, Err(GitError::NotGitRepo)));
    }
    ...
    fn test_parse_github_kind() {
        ...
        assert_eq!(
            GitContext::parse_repo_kind("https://cnb.cool/owner/repo.git"),
            CodePlatform::CNB
        );
        assert_eq!(
            GitContext::parse_repo_kind("git@cnb.cool:owner/repo.git"),
            CodePlatform::CNB
        );
        assert_eq!(
            GitContext::parse_repo_kind("https://codeup.aliyun.com/owner/repo.git"),
            CodePlatform::Codeup
        );
        assert_eq!(
...
src/generated
"#;
        std::fs::write(tmp.path().join(".gitignore"), gitignore_content).unwrap();

        let ctx = GitContext::init(tmp.path()).unwrap();
        let patterns = ctx.get_ignore_directory_patterns();

        // 应该包含来自 .gitignore 的目录模式
        ...
        assert!(patterns.contains(&"src/generated".to_string()));

        // 不应该包含文件模式（*.log, **/*.pyc）
        assert!(!patterns.iter().any(|p| p.contains('*')));

        // 不应该包含否定模式（!important.log）
        assert!(!patterns.iter().any(|p| p.starts_with('!')));

        // 应该包含常见的大型目录（即使 .gitignore 中没有）
